 ...
 *
 * 판정은 `node:net`의 내장 `BlockList`가 한다. 외부 IP 파싱 라이브러리(`ipaddr.js`)를
 * 넣지 않는 근거는 §4·§7이다: **재판정이 필요한 형식이 well-known NAT64 하나뿐이고
 * 그것이 표 확장으로 표현되므로**(아래 `BLOCKED_NAT64_SHADOW` — 파싱 코드 0줄),
 * 나머지는 통째 차단이라 추출할 대상이 없다. 의존성 예산 2개(§2)를 쓰지 않는다.
 *
 * 내장이 대신 처리하는 것은 **IPv4-mapped `::ffff:0:0/96` 한 형식뿐이다**(2026-08-08
 * 실측). 그 한 형식에 대해서만 우리 코드에 방어가 없고 내장 동작에 의존한다 — 그래서
 * §8이 그 의존을 계약 테스트로 고정했다. 테스트가 깨지면 명시적 정규화를 우리가 떠안는다.
 * **다른 임베드 형식은 내장이 덮지 않으며**(실측: `BlockList.check("64:ff9b::a9fe:a9fe",
 * "ipv6")`가 `false`), 그 처분은 아래 두 표가 직접 든다(§4 「IPv4를 품은 IPv6 — 두 규칙」).
 *
...
/**
 * IPv6 차단 대역. **이 표의 여섯(규칙 2 구성원)을 지배하는 것은 §4 「IPv4를 품은 IPv6 —
 * 두 규칙」이고, 나머지 일곱은 §4 「차단 대역」 열거가 직접 지배한다.**
 *
 ...
 *    로컬 전용 형식에는 재판정이 살릴 것이 없다. **아래 열거 중 IPv4를 품은 여섯**
 *    (`64:ff9b:1::/48`·`::/96`·`::ffff:0:0:0/96`·`2002::/16`·`2001::/32`·`2001:db8::/32`)이
 *    이 규칙이다. **나머지 일곱**(`fc00::/7`·`fe80::/10`·`fec0::/10`·`ff00::/8`·`100::/64`·
 *    `2001:2::/48`·`2001:20::/28`)은 IPv4를 품지 않는 별개의 차단 사유이며 §4 「차단 대역」
 *    열거에서 직접 온다 — 이 두 규칙이 다루는 대상이 아니다.
 *
