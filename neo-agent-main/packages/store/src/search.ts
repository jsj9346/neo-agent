 ...
 * 지배한다.
 *
 * [미규정 ES-33] **chainTip 해석은 재귀 CTE 1회이고 애플리케이션 측 해석이 아니다**
 * (SEARCH §8의 미결). 같은 실측에서 앱 측 해석이 다건 질의에서 8배 빨랐다(6.4ms vs
 ...
 *
 * [미규정 ES-34] **폴백의 정렬은 `timestamp` 내림차순이다**(QA-A A-7). §4가 정한
 * "rank(bm25) 기본"은 이 경로에 적용할 수 없다 — MATCH를 쓰지 않으므로 bm25가 없다.
...
/**
 * [미규정 ES-36] **0 이하·비정수 `limit`은 기본값으로 취급한다**(QA-A A-8).
 *
