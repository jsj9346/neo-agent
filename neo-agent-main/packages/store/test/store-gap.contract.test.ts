 ...
 *
 * **이 파일은 구현보다 먼저 쓰였다.** `SESSION-STORE.md` §5가 스스로 선언한 대로
 * ("⚠️ 계약-구현 격차 (의도적)"), 두 계약은 2026-08-06 CLI 설계에서 확정된 문서상
 * 계약이며 `packages/store` 구현에는 아직 없다. 따라서 **여기의 실패는 정상이고,
 * 계약을 고정하는 것이 이 파일의 목적이다** — 메시지 id(O-4) 때와 같은 처리다.
 *
 * 기대값의 출처:
 ...
 *
 * **기대값은 구현 코드에서 도출하지 않았다.** 특히 현재 `sessions.ts`의
 * `resolveSessionId`는 "`active` 여부로 거르지 않는다"를 주석으로 명시하고 있으나,
 * §5가 "삭제된 세션은 … resolveSessionId에서 제외"라고 규정하므로 **테스트는 문서
 * 편이다**. 구현 주석의 근거(지워진 세션의 접두가 "없는 세션"으로 보이는 문제)는
 * 재론할 가치가 있으나, 재론은 문서 개정으로 하고 테스트는 현행 정본을 고정한다.
 *
