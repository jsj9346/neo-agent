 ...
 *
 * 600 자체가 아니라 group·other 비트로 판정한다. SAFE-DEFAULTS §3.1의 괄호가
 * 판정 기준을 말하고 있고("600(소유자 외 접근 가능)이 아니면"), 그 기준이면
 * `0400`처럼 **더 좁은** 권한은 통과해야 한다 — 더 안전하게 잠근 사용자를
 * 기동 거부로 벌하는 것은 계약의 목적과 반대다.
 * [미규정] 정확히 `!== 0o600`으로 볼 것인지의 선택. 위 근거로 노출 비트 검사로 닫았다.
 */
