 ...
 * denylist가 파일 도구 3종을 전부 막고, 그래서 **파일 도구 쪽에서** `remember`가
 * 유일한 문이 된다. 이 경로를 열면 사용자가 워크스페이스를 가리킬 수 있고 그 순간
 * `write_file` 한 번으로 예산·표시·오염 정책이 전부 우회된다.
 *
 * **셸은 이 판정을 지나지 않는다** (2026-09-02 — `K-427`). `shell`은 `cwd`만 경계
 * 검증을 받고 `command` 문자열은 경로 판정을 안 거치므로, 호스트 실행에서는 이 경로가
 * 읽기·덧붙이기 양쪽으로 열린다. 셸에 대한 격리 소유자는 이 denylist가 아니라
 * 샌드박스이며(`docs/SANDBOX.md` §6 — 기본 구성에서 `~/.neo-agent/`는 컨테이너 안에
 * 없다), 열려 있는 갈래는 명시적 `sandbox: "off"` 하나다. 범위의 정본은
 * `docs/MEMORY.md` §2.1 「한계 — 셸은 이 문을 지나지 않는다」.
 *
