...
export function createRepl(io: TerminalIo, handlers: ReplHandlers): Repl {
   ...
   * 상태줄을 그리는가 — 축이 둘이다.
   *
   * 1. `input.isTTY === true` — raw 모드·키 처리·커서 제어의 전제(`docs/CLI-INTERFACE.md`
   *    §7.1 그리기 게이트의 첫 축).
   * 2. `output.columns`가 유한한 양수 — 폭을 알아야 자를 수 있다(같은 게이트의 둘째 축 —
   *    1행 제약).
   *
   * 두 축은 §7.1이 명문화한 게이트다(2026-08-21 확정 — 그전까지 대상 스트림이
   * 미규정이었고, 이 구현의 좁은 해석을 그 절이 추인했다). 폭은 출력 스트림의 성질이라
   * 입력 축이 대신하지 못한다: `main.ts`가 막는 것은 `process.stdin.isTTY`뿐이라
   * `neo-agent | tee log`는 기동하고, 그때 stdin은 TTY인데 stdout은 파이프라 `columns`가
   * 없다. 그 조합에서 그리면 무제한 폭으로 감싸이고 `statusRows()`가 그 즉시 틀어진다.
   *
