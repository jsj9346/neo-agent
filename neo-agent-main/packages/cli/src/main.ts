 ...
 *
 * **argv를 보지 않는다.** 그래서 `--version`·`--help`도 함께 거부된다 — 순서를 지키려
 * 애쓴 결과가 아니라 이 파일이 argv를 해석하지 않는다는 기존 계약(§2: 해석은 조립의
 * 일이다)의 자연스러운 귀결이고, 플랜 §9 D-1이 그것을 수용된 귀결로 확정했다.
 *
 ...
 * §7.1이 "닫아도 검증 경로를 막지 않는다"고 판정한 근거다.
 */
