 ...
 *
 * **이 검사가 argv에 대해 여는 예외는 닫힌 목록이고, 이 파일은 그 수를 지지 않는다**
 * (2026-08-24 신설 · 2026-09-06 `doctor` 편입 — `CLI-INTERFACE.md` §2.2 계약 4·5).
 * 목록과 갈래별 근거의 정본은 그 절의 표이고 여기에 옮겨 적지 않는다 — 수를 세는
 * 것은 파서의 갈래이지 이 주석이 아니다. 면제 판정은 **조립이 소유한 파서에 갈래를
 * 물어** 얻는다 — 여기서 argv를 손으로 비교하는 것은 그 절이 이름으로 금지했다
 * (argv를 읽는 것이 둘이 되면 갈리는 방향 하나가 침묵 실패다). 파싱이 던지거나
 * 면제 밖 갈래로 답하면 검사를 건다.
 *
 * **면제는 갈래마다 자기 근거를 갖고, 근거가 죽으면 그 면제도 죽는다**
 * (`WEB-UI.md` §3 · `CLI-INTERFACE.md` §2.2 계약 5의 표 · 같은 문서 §5.1 계약 8).
 * 이 거부가 막는 것은 «승인 게이트가 물어볼 사람이 없는데 묻는» 상태이므로, 면제의
 * 근거는 그 상태가 그 갈래에서 성립하지 않는다는 것뿐이다 — 편의는 근거가 아니다.
 * 두 갈래가 그것을 서로 다른 방식으로 만족하고, **`doctor`의 근거가 더 짧다**:
 *
 * - `serve` — **승인자의 이전.** 묻는 상대가 터미널이 아니라 브라우저다. 그래서
 *   `WEB-UI.md` §7의 귀속이 바뀌어 승인자가 브라우저가 아니게 되면 이 면제는 그
 *   순간 근거를 잃는다.
 * - `doctor` — **승인의 부재.** 도구를 실행하지 않으므로 승인 게이트가 애초에 서지
 *   않는다(§5.1 계약 5·8). 그래서 `doctor`가 승인을 묻는 일을 하게 되면 이 면제는
 *   그 순간 근거를 잃는다.
 *
 * 근거가 그 명령에서 죽으므로 결론도 함께 죽는다 — **거부를 없앤 것이 아니다.**
 *
...
if (process.stdin.isTTY !== true && !isExemptFromTtyRequirement(process.argv.slice(2))) {
  process.stderr.write(
    [
      ...
      "  참고       무인 실행(cron·스크립트·파이프)은 아직 지원하지 않습니다. --version·--help도 같은 이유로 거부됩니다. 예외로 웹 UI 서버(neo-agent serve)는 승인을 브라우저에 묻고, 진단(neo-agent doctor)은 승인을 묻지 않아 이 거부에 걸리지 않습니다.",
      "",
    ].join("\n"),
  );
 ...
 *
 * **자기 파서를 만들지 않는다**(계약 3). argv를 손으로 비교해 면제 갈래를 알아내면
...
function isExemptFromTtyRequirement(argv: readonly string[]): boolean {
  ...
  // 면제 갈래의 정본은 §2.2 계약 5의 표다. 여기는 그 표의 갈래 이름만 열거하고
  // 근거는 위 블록이 든다 — 갈래가 늘면 그 표가 먼저 늘고 이 줄이 따라온다.
  return parsed.kind === "serve" || parsed.kind === "doctor";
}
