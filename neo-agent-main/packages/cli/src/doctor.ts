 ...
 * 죽는」 형태로만 관측된다.
 *
 * **출력에 ANSI를 섞지 않는다**(계약 7·8). 이 명령의 정상 경로는 파이프이고 TTY 없이
 * 도는 것이 계약이므로, 색을 섞으면 자동 점검이 받는 것이 이스케이프 낀 문자열이 된다.
 */
...
/**
 * 보고서를 화면 문자열로. **ANSI를 섞지 않고 모든 축이 한 줄 이상을 갖는다** — 판정이
 * 화면에 안 나오는 축이 있으면 그것이 곧 침묵 실패다(`ARCHITECTURE.md` §2.6).
 *
 * **[미규정 DR-3]** §5.1은 *"보고서 렌더링"*을 이 파일의 몫으로만 두고 표기 형태도 이
 * 함수의 이름·시그니처도 정하지 않았다. 문면은 §7의 표시 문구 규약대로 세부로 두고,
 * 계약으로 지는 것은 「축마다 판정이 보인다」와 「`skipped`는 이유를 함께 낸다」뿐이다.
 */
