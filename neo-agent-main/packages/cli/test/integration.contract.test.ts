...
/* ═══════════════════════════════════════════════════════════════════════════
 * [미규정] 판정 요청 — 이 파일이 임의로 정하지 않은 것 (QA-B / T-011)
 *
 * 보고는 유실될 수 있으므로 여기에 남긴다.
 *
 * ── I-1: 재개 직후 과거 대화의 **표시 범위** ────────────────────────────────
 *   §6은 "표시 범위(마지막 몇 턴)는 구현 세부이되, 재개 직후 '어디까지 진행된
 *   세션인지'가 화면에 보여야 한다는 것이 계약"이라고 한다. 계약과 세부의 경계가
 *   "무엇이 보이면 계약을 이행한 것인가"에서 갈린다 — 마지막 1턴이면 충분한가,
 *   세션 id·title·메시지 수 같은 요약만으로도 되는가. 테스트는 "재료가 손에
 *   들어온다"까지만 단언하고 표시 판정은 wiring 쪽에 열어 두었다.
 *
 * ── I-2: 과거 대화를 그리는 함수의 소유 모듈 ────────────────────────────────
 *   `renderer.ts`가 내보내는 것은 `createRenderer(out): AgentEventListener`
 *   하나이며, **트랜스크립트 배열을 받아 그리는 표면이 공개 배럴에 없다.**
 *   §6은 "렌더러는 loadSession 반환 배열로 직접 그린다"고만 하고 그 표면이
 *   어디 사는지를 정하지 않는다(renderer인지 wiring인지). 이 빈칸은 T-008이
 *   메꾸겠지만, 계약상 어느 쪽이 소유하는지는 판정이 필요하다.
 *
 * ── I-3: 승인 대기 중 렌더러 출력과 프롬프트 출력의 싱크 공유 ────────────────
 *   §9는 "승인 대기와 모델 스트리밍은 겹치지 않는다"고 하므로 한 싱크를 공유해도
 *   충돌하지 않는다. 다만 **둘이 같은 싱크여야 한다는 규정은 없다** — 하네스는
 *   같은 싱크로 조립했고(실제 CLI가 그럴 것으로 읽었다), 다르게 조립할 근거가
 *   생기면 시나리오 2의 `display` 검사 위치가 바뀐다.
 *
 * ── I-4: 게이트 block 사유의 언어 검사 강도 ─────────────────────────────────
 *   §4는 `reason`을 "영어"로 규정한다. 테스트는 한글이 섞이지 않는지까지만 본다 —
 *   "영어인가"를 기계적으로 판정할 수단이 없고, 재는 대상은 **CLI가 자기 사유를
 *   덧붙였는가**이기 때문이다. 더 강한 판정 기준이 필요하면 지정해 주기 바란다.
 * ═══════════════════════════════════════════════════════════════════════════ */
