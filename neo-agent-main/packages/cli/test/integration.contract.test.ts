 ...
 *
 * I-1·I-3·I-4는 **2026-08-06에 판정됐다.** 판정 정본은
 * `plans/20260806-cli-qa-report.md` §4(조치 불필요 15건)다. 열린 판정이 아니므로
 * 미규정 마커를 달지 않는다 — **닫힌 항목이 마커를 유지하면 진짜 열린 항목이 묻힌다.**
 * **I-2는 예외로 열려 있다**(아래 — 판정의 전제가 거짓이었다).
 *
 ...
 *
 * ── [미규정] I-2: 과거 대화를 그리는 함수의 소유 모듈 (= W-4) — **열려 있다** ──
 *   물음: §6은 "렌더러는 loadSession 반환 배열로 직접 그린다"고만 하고 그 표면이
 *   어디 사는지를 정하지 않는다(renderer인지 wiring인지).
 *   2026-08-06 판정은 "내부 전용 확정 — 배럴에 올리지 않는다"였으나
 *   (`plans/20260806-cli-qa-report.md` §4), **2026-08-11 독립 QA 실측으로
 *   `src/index.ts`가 `renderTranscript`를 실제로 재수출함이 확인됐다.** 판정이
 *   전제한 "배럴에 없다"가 판정 시점에 이미 거짓이었으므로 이 항목은 닫히지
 *   않는다. 상세와 선택지는 `wiring.contract.test.ts` 말미의 W-4에 있다(같은
 *   항목이므로 **정정은 반드시 두 파일 동시에**).
