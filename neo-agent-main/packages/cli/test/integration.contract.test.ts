 ...
 *
 * 넷 다 **2026-08-06에 판정됐다.** 판정 정본은 `plans/20260806-cli-qa-report.md`
 ...
 *   물음: `renderer.ts`가 내보내는 것은 `createRenderer(out): AgentEventListener`
 *   하나이며 트랜스크립트 배열을 받아 그리는 표면이 공개 배럴에 없다. §6은
 *   "렌더러는 loadSession 반환 배열로 직접 그린다"고만 하고 그 표면이 어디 사는지를
 *   정하지 않는다.
 *   **판정 — 내부 전용 확정** (2026-08-06). `renderTranscript`는 §6의 이행 세부이고
 *   소비자는 wiring 하나(같은 패키지)이므로 배럴에 올리지 않는다. 배럴에 없는 것이
 *   계약 위반이 아니라 **계약의 결론**이다.
 *
