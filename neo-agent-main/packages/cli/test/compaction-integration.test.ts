 ...
 * 회색지대는 임의 판정하지 않고 파일 말미에 모았다(B-12부터). 그 셋은 **2026-08-06에
 * 전부 판정됐고**(정본 `plans/20260806-compaction-qa-report.md` §3) 말미 블록은 이제
 * 판정 요청이 아니라 **판정 기록**이다 — 이 파일에 열려 있는 항목은 없다.
 *
 ...
 */
describe("시나리오 2-b — 미지 모델의 보수 기본값이 판정에 쓰인다 (PROVIDERS §3 R6)", () => {
...
/* ═══════════════════════════════════════════════════════════════════════════
 * 판정 기록 — T-012에서 드러났던 것과 그 처분 (B-12부터)
 *
 * 셋 다 **2026-08-06에 판정됐다.** 판정 정본은
 * `plans/20260806-compaction-qa-report.md` §3이며, B-13·B-14는 `devlog.md`
 * 같은 날 항목에도 결정으로 올라 있다. 열린 판정이 아니므로 미규정 마커를 달지
 * 않는다 — **닫힌 항목이 마커를 유지하면 진짜 열린 항목이 묻힌다.**
 * B-1~B-11은 `packages/store/test/`의 두 파일에 있다.
 *
 * ── B-12: 재개 직후 압축이 **방금 그린 트랜스크립트를 즉시 폐기한다** ──────────
 *   물음: `wiring.ts`의 `run()`은 재개 트랜스크립트를 전부 렌더한 뒤
 *   `compaction.auto()`를 부른다(주석의 근거: "어느 대화에 접속했는지를 먼저 보고
 *   나서 압축 사실을 봐야 순서가 읽힌다"). 그런데 재개 직후 판정이 참이면 그 대화는
 *   그 자리에서 자식 세션으로 대체된다 — 사용자는 화면을 가득 채운 과거 대화를 본
 *   직후 "새 세션으로 이어간다"를 본다. R-5 스모크(메시지 600건)에서 실제로 600건을
 *   그린 뒤 압축이 일어났다. COMPACTION §3·§6은 판정 시점과 표시 의무만 정하고
 *   렌더와 압축의 **선후**는 정하지 않는다.
 *   **판정 — 현행 유지** (2026-08-06). 지금 순서가 §6의 재개 표시 의무와 판정 시점
 *   계약을 **둘 다** 지키는 유일한 순서다. 제시됐던 (b)"판정 먼저·압축될 대화는
 *   그리지 않는다"·(c)"재개 시 요약만"은 표시 의무 쪽을 깎는다.
 *   **이월된 것은 B-12가 아니라 렌더 절약이다** — 그 축은 `CLI-INTERFACE.md` §12의
 *   미결 "표시 세부 — 재개 시 과거 대화 표시 범위"와 **함께 재론**한다(리포트가
 *   그렇게 적었다). 즉 여기서 닫힌 것은 선후이고, 열려 있는 것은 §12의 표시 범위다.
 *
 * ── B-13: 마이그레이션이 **완전히 침묵한다** ───────────────────────────────
 *   물음(당시 v1 → v2): 실측(2026-08-06, 임시 probe로 확인 후 삭제) 구 버전 DB로
 *   기동하면 스키마가 승격되지만 화면에 어떤 표시도 없었다. `SESSION-STORE.md` §2는
 *   마이그레이션 **규율**(앞으로만·한 트랜잭션·상위 버전 거부)만 정하고 표시 의무를
 *   정하지 않는다. 그러나 마이그레이션은 되돌릴 수 없는 변경이므로 침묵 금지
 *   (ARCHITECTURE §2.6)의 방향과 견주면 판정이 필요했다.
 *   **판정 — 통지 채택, 구현 보강** (2026-08-06 · `devlog.md` 같은 날 결정).
 *   **신규 DB 생성은 통지하지 않는다** — 생성은 승격이 아니고 매 신규 설치가 경고로
 *   시작하는 것은 §2.3에 어긋난다. 구현 실물은 `packages/store/`에 있다:
 *   `src/migrate.ts`의 `onUpgrade` 훅과 `src/open.ts`의 `SchemaMigratedWarning`
 *   (`kind: "schema-migrated"`, `from`·`to` 포함). 그것을 덮는 테스트도 같은 패키지
 *   (`test/open.test.ts`의 신규-생성/승격 두 갈래, `test/schema-v3.contract.test.ts`)에
 *   있으므로 **이 파일이 표시 유무를 단정하지 않는 것은 여전히 옳다** — 위 시나리오 8이
 *   결과(데이터 보존·재개 성공)만 단정하는 배치를 바꾸지 않았다.
 *
 * ── B-14: 취소가 연속 실패 **streak을 끊는가** ──────────────────────────────
 *   물음: A-1이 "취소는 실패 카운트에 넣지 않는다"를 정했고 구현도 그렇다
 *   (`compact.ts`의 abort 분기는 `consecutiveFailures`를 건드리지 않고 return한다).
 *   그런데 리셋도 하지 않으므로 실패 → 취소 → 실패가 "연속 2회"가 된다.
 *   **판정 — "연속"의 기준은 성공이다** (2026-08-06 · `devlog.md` 같은 날 결정).
 *   취소는 실패 카운트를 늘리지도 **끊지도** 않는다 — 취소는 실패 원인이 해소됐다는
 *   증거가 아니므로, 끊으면 실패가 지속되는 상황에서 취소 한 번마다 스래싱이
 *   되살아난다. 현행 유지 + **`COMPACTION.md` §7에 명문화**됐다(실물 확인).
 *   위 A-1 테스트가 **취소만 2회**인 경우만 단정하는 것은 그대로 둔다 — 실패-취소-실패
 *   축의 기대값은 §7이 갖고 있다.
 * ═══════════════════════════════════════════════════════════════════════════ */

// ═══════════════════════════════════════════════════════════════════════════
// 종료 시퀀스의 대기 지점 — Ctrl+D가 무엇을 기다리는가 (CLI-INTERFACE §2 · §8)
// ═══════════════════════════════════════════════════════════════════════════

/**
 * §2가 종료 시퀀스를 이렇게 연다: *"`waitForIdle()` → **인플라이트 압축 대기** →
 * `store.close()` → 세션 id와 재개 방법을 표시하고 종료."* 앞의 **둘이 대기 지점**이고,
 * 아래 두 테스트가 각각 하나씩 잰다. §8은 같은 사실을 상태별로 적는다 —
 * `run-active` 행이 첫 번째를, `compacting` 행이 두 번째를 가리킨다.
 *
 * **왜 대기 지점이 둘인가 — 하나가 다른 하나를 대신하지 않기 때문이다.**
 * `waitForIdle()`은 agent를 본다. 그런데 자동 압축은 `agent.prompt()`가 resolve된
 * **뒤** 불리므로(`wiring.ts:637-640` — 판정 E-45의 의도된 배치) 압축이 도는 동안
 * agent는 이미 idle이고, `waitForIdle()`은 압축에 대해 아무것도 말하지 않는다.
 * 즉 첫 대기 지점은 **런**을, 두 번째는 **agent 밖에서 도는 런의 꼬리**를 본다.
 * 한쪽만 있으면 §2가 절반만 지켜지며, 실제로 그런 시기가 있었다 — 이 줄이
 * `waitForIdle → store.close`였던 동안 §8 `compacting` 행은 거짓이었다(13사이클 실측).
 *
 * 두 대기 지점을 한 describe에 두는 이유가 그것이다. 나란히 놓지 않으면 다음에
 * 종료 시퀀스를 고치는 사람이 **자기가 건드린 쪽만** 보게 된다.
 *
 * 어느 쪽이 어긋나든 결과는 조용하다. 압축 쪽은 `store.close()`가 `branchSession`보다
 * 먼저 가고(`compact.ts`), 압축 컨트롤러는 §7에 따라 자기 실패를 삼키므로(던지면 REPL이
 * "런 실패"로 표시해 실패의 출처가 어긋난다) **사용자는 요약이 사라진 것을 모른다.**
 * 런 쪽은 응답이 저장되기 전에 프로세스가 끝난다. `ARCHITECTURE.md` §2.6의 심각도
 * 순서에서 둘 다 가장 나쁜 쪽이고, 그래서 두 테스트 모두 *"기다렸는가"*에서 멈추지 않고
 * **버려졌는가**(분기 세션 / assistant 행)를 함께 단정한다.
 *
 * REPL 계층은 여기서 재지 않는다 — 키 처리와 "요약 signal을 끊지 않는다"는
 * `input.test.ts`가 보고, **기다림 자체는 배선 계층 사실**이라 이 파일이 잰다.
 */
describe("종료 시퀀스의 대기 지점 (CLI-INTERFACE §2)", () => {
  /**
   * §2가 종료 시퀀스의 **첫** 대기 지점으로 적은 것: *"`waitForIdle()` → 인플라이트
   ...
   */
  it("종료 시퀀스는 진행 중인 런을 기다린다 — 응답을 버리지 않는다 (§2 · §8 `run-active`)", async () => {
    let release!: () => void;
