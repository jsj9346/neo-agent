 ...
 *   - §3.1 «코드 주석은 이 판에서 안 잰다»(§9 U-b) · §9 U-a·U-f — 정본이 스스로 안 닫은 자리
...
describe("PUBLIC-TREE §3.1 — 축 3: 글롭 별표", () => {
  /** *"**첫 별표 앞의 마지막 경로 구분자까지로 잘라** 남은 접두를 판정하고,"* */
  it.each([
...
describe("PUBLIC-TREE §3.2 — 축 5: 해결 순서 3단", () => {
  ...
  /**
   * §3.2 — *"**맨 이름 꼴은 디렉터리를 걷지 않는다.** 공개 트리에 추적된 `.md`의
   * **파일명 색인**에서 찾는다."*
   */
...
describe("PUBLIC-TREE §3.2 — 축 6: 그림자가 실패인 자리와 아닌 자리", () => {
   ...
   * 뜻하는 것은 **자리이지 대상이 아니고**, 첫 적중(인용한 문서가 사는 트리)이 정확히 쓰는
   * 사람이 뜻한 자리다."*
   */
  ...
  /**
   * *"예외 목록이 아니라 판정이다 — 둘 다 술어가 「부류가 갈리는가」 하나이고, 갈리는 자리는
   * 실패한다."*
   */
...
describe("PUBLIC-TREE §3.3 — 축 8: dead-section은 같은 스팬 안에서만 발화한다", () => {
   ...
   *
   * §4.2는 *"정본 셀은 포인터 의무다."*라고 적고, 그 뒤에 *"그 §의 실재는 §3.3의
   * `dead-section`이 잰다."*고 이었다. 그런데 같은 절이 정한 정본 셀의 문법은 *"백틱으로
   * 감싼 문서명 하나 + `§<절번호>`"* — 즉 **§ 번호가 스팬 밖**이다. 좁은 범위(같은 스팬)의
   * `dead-section`은 정확히 그 꼴에 반응하지 않으므로, §4.2의 저 문장은 실물에서 거짓이다.
   *
...
describe("PUBLIC-TREE §3.2 — 축 9: 트리 부재는 통과가 아니라 미판정이다", () => {
  /**
   * *"**해결할 트리가 없으면 통과가 아니라 미판정이다.** clone한 자리에는 비공개·동결 트리가
   * 없다. 그 주소들을 조용히 통과시키면 게이트가 실행 환경에 따라 다른 것을 재면서 같은
   * 초록을 낸다 — `ARCHITECTURE.md` §2.6이 금지하는 침묵 경로다."*
   */
