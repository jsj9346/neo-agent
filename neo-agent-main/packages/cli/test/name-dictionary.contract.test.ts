 ...
 * | §4.2 | 절 범위는 `## 4.`와 다음 첫 `###` 사이 | 대조 1/1 — 축 6 |
 * | §4.2 | 실패 갈래 여섯 | 대조 6/6 — 축 4 |
 * | §4.2 | 갈래 이름 = 출력 라벨 | 대조 1/1 — **[추정] 해소.** 머리의 조정 가능 목록이 실패 메시지 문구를 든다 → 문자열은 계약이 아니다. 축 0 |
...
describe("LORE §4.2 — 축 0: 표면", () => {
  ...
  it("라벨 «문자열»은 계약이 아니다 — 이 파일은 심볼로만 지목한다", () => {
    // **[추정] 해소.** 플랜은 **갈래 이름이 곧 출력 라벨**이라는 문자열 동일성을 [추정]으로 남겼는데,
    // 이 문서 머리가 그 자리를 이미 정하고 있다 — 조정 가능 목록이
    // *"§4.2 게이트의 실패 메시지 문구와 스크립트의 파일 위치"*를 든다. 즉 **문구는 개정
    // 없이 바꿀 수 있는 층**이고, 여기서 글자로 못박으면 이 테스트가 문서가 허용한 변경을
    // 막는다. 그래서 축 4~축 9는 전부 `VIOLATIONS.*` 심볼로 지목하고, 여기서는 **닫힌 집합**
    // (§4.2 표와 1:1)만 잰다.
    const labels = Object.values(VIOLATIONS);
...
// 축 6 — §4.2 절 범위
//
// *"절 범위는 `## 4.`와 그 다음에 처음 나오는 `###` 사이다"*. §4.1의 예시 표 둘이 정확히
// 그 함정이고, `DOC-STATUS.md` §5.1이 자기 소절의 표에 데인 자리다.
// ---------------------------------------------------------------------------
