 ...
 * 막는 모듈 집합과, 그 패키지의 설계 문서가 명시 열거한 금지 모듈 집합이 **열 패키지
 ...
 *
 * 이 검사는 한 패키지가 아니라 **열 패키지 전부와 게이트**를 대상으로 하는 **횡단 검사**다.
 ...
 * `providers/test/`에 두면 그 패키지 것이 아닌 아홉 패키지의 계약을 그 패키지가 소유하게 된다.
 *
 ...
 * - **문서 쪽**: 아래 `DOC_BY_PACKAGE`가 드는 열 문서의 **금지 선언 줄**.
 ...
 * | R-7 앵커는 `**금지 내장 모듈**`과 다른 문자열 | 축 5 역검증 ④ |
 ...
 * ## 「10」은 세 자리를 묶는 결합이다
 *
 * `EXPECTED_PACKAGE_COUNT`가 10이 아니면 이 파일이 스스로 실패한다. 패키지가 늘거나 줄면
 ...
 *
 * 1. `docs/` 아래 설계 문서 — 아래 `DOC_BY_PACKAGE`가 드는 열 건과 `ARCHITECTURE.md` §2.6.
 ...
 *
 * R-7 — `**금지 내장 모듈**`은 이 문자열과 갈린다. `docs/WEB-UI.md`에 그 형태의 아홉 열거가
 * 따로 살아 있으나 그 패키지는 게이트 `PACKAGES`에 없어 이 대조의 모수 밖이다.
 */
const ANCHOR = "- **금지 모듈**: ";
...
describe("문서 추출 규약 — R-1 · R-2 · R-6 · R-7", () => {
  ...
  it("R-7 — `**금지 내장 모듈**` 형태의 줄은 읽지 않는다", () => {
    // `docs/WEB-UI.md`에 그 형태가 살아 있고 그 패키지는 게이트 PACKAGES 밖이다.
    // 앵커가 그 문자열과 갈리므로 충돌하지 않는다는 것을 표본으로 고정한다.
