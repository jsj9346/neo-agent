...
describe("DOC-STATUS §5.1 — 축 6-6: 표 실패 사유는 §3.1의 일곱과 다른 이름공간이다", () => {
  ...
  it("두 유니온이 서로를 받아들이지 않는다 — 이 케이스는 tsc가 잰다", () => {
    ...
    // 합쳐지면 축 4의 전수 단언이 표 실패까지 세게 되어, 머리말의 불변 선언이 드는 «머리 판정 §3.1»
    // 갈래가 조용히 넓어진다(2026-08-17 — 머리가 수를 빼고 갈래를 §포인터로 들도록 개정됐다.
    // 그 전 문면은 *"일곱 가지 실패"*였다). **런타임 단언이 아니다** — `@ts-expect-error`는
    // 에러가 없으면 컴파일이 실패한다.
