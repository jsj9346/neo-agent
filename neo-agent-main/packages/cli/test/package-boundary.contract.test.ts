 ...
 *
 * **다만 이 파일은 그 렉서를 임포트하지 않고, 이 머리가 그 이유를 든 적이 없다.** 같은
 * 술어를 복제한 형제 다섯(`memory`·`web`·`tools`의 같은 이름 파일, `sandbox`의
 * `boundary.contract`, `gate`의 `hook-surface.contract`)은 저마다 `typescript` 파서 비용을
 * 이유로 든다. **그 근거는 이 자리에 그대로 서지 않는다** — 같은 디렉터리의
 * `renderer-literal-policy.qa.test.ts`와 `first-run-rename.qa.test.ts`가 그 렉서를
 * 임포트한다. [미규정] — 이 자리의 이유는 아직 판정되지 않았고 여기서 지어내지 않는다.
 *
