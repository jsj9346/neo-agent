 ...
 *
 * 기대값의 출처는 전부 §1이다:
 *   - §1:15~16 — 패키지 위치·bin 이름·**의존성 예산**(워크스페이스 10패키지 정확히, 외부 0)
 *   - §1:20 — **금지 모듈 8종 / 허용 5종.** 이 파일은 허용 쪽을 닫는다(아래 축 3 참조).
 *     금지가 여섯에서 여덟이 된 것은 2026-08-24 개정이다 — `node:dgram`·`node:worker_threads`가
 *     열거에 들어왔고, 게이트는 그전부터 둘을 막고 있었다
 ...
 *
 * **그 판정이 걸리는 것은 두 층 중 판정 층뿐이다.** 이 레포는 렉서는 공유하고 판정은
 * 복제한다 — 「무엇이 한 주석인가」의 정본은 `scripts/comment-lexer.mjs`이고, 그 모듈이
 * `렉싱 수단은 확장자가 고르고, 그 분기는 한 자리에 둔다`(`DOC-CITATION.md` §6 U-b)를
 * 근거로 확장자별 렉싱 분기를 `EXTENSION_LEXERS` 한 자리에 모은다. 「그래서 이 계약이
 * 충족인가」는 각 계약 테스트가 자기 헬퍼로 든다 — 위 문단이 의도라고 적은 복제가 그쪽이다.
 * 주석 우회를 닫을 때는 어느 층을 만지는지부터 가른다.
 *
 * **이 파일은 그 렉서를 임포트하지 않고, 그 사실은 이유를 요구하지 않는다.** 같은 술어를
 * 복제한 형제 다섯(`memory`·`web`·`tools`의 같은 이름 파일, `sandbox`의
 * `boundary.contract`, `gate`의 `hook-surface.contract`)은 저마다 `typescript` 파서 비용을
 * 이유로 드는데 **그 근거는 이 자리에 안 선다** — 같은 디렉터리의
 * `renderer-literal-policy.qa.test.ts`와 `first-run-rename.qa.test.ts`가 그 렉서를
 * 임포트한다. 그 자리를 2026-08-23에 `DOC-CITATION.md` §6 U-b가 닫았다: **판정 층은 복제가
 * 기본값이고 기본값에는 이유를 대지 않는다.** 렉서 층에서 한 자리에 두는 것은 확장자 분기이고
 * 이 파일의 헬퍼는 고를 것을 갖지 않으므로 그 판정의 모집단 밖이다. 이유를 요구받는 것은 반대
 * 방향뿐이다 — 분기를 새로 만드는 자리다.
 *
...
// 축 2 — 의존성 예산 (§1:15~16)
//
// §1:16은 접속사로 이어진 **두 사실**이다: *"워크스페이스 10패키지 정확히"* **그리고**
...
// 11패키지 합계를 `DISTRIBUTION.md`에 대고 재고, 이쪽은 `cli` 하나를 `CLI-INTERFACE.md`
...
describe("CLI-INTERFACE §1 — 축 2: 의존성 예산", () => {
  ...
  it("런타임 의존성이 워크스페이스 10패키지와 정확히 일치한다 (§1:16 앞절반)", () => {
  ...
  it("패키지가 private: true다 (§1:26의 전건)", () => {
    ...
    //
    // cli 하나만 잰다 — 이 파일의 정본(§1:26)이 말하는 것이 cli이기 때문이다. 전 패키지
...
//
// §1:20은 두 목록을 적는다: 금지 8종(`net`·`tls`·`http`·`https`·`child_process`·`sqlite`·
// `dgram`·`worker_threads`)과 허용 5종(`fs`·`os`·`path`·`readline`·`tty`). **예산 게이트는
// 금지 쪽만 검사하므로 목록 밖의 것은 통과한다** — 오늘 `cli`가 `node:dns`를 임포트해도
// `pnpm check:budget`은 그린이다 (`memory`·`compaction`의 금지 목록에는 dns가 있는데
// `cli`에만 빠져 있다. 실측 확인).
//
// dns를 금지 목록에 더해 막을 수도 있지만 그러면 다음 구멍(`vm`·`v8`·`inspector`…)마다 같은
// 일을 반복한다. §1:20이 **허용 5종을 이미 열거해 뒀으므로** 방향을 뒤집으면 목록 밖 전부가
// 한 번에 닫힌다. 새 계약을 만드는 것이 아니라 이미 적힌 것을 재는 것이다.
//
// **이 문단이 처음 쓰일 때 「다음 구멍」의 예로 들었던 `dgram`·`worker_threads`는
// 2026-08-24에 실제로 금지 열거로 올라갔다.** 그것이 위 논증을 무르지는 않는다 — 그 개정도
// 우회 경로가 되는 둘만 골라 **명시 금지로도** 올린 것이고, 목록 밖 전부를 한 번에 닫는 것은
// 여전히 아래 허용 목록 폐쇄뿐이다. 두 축이 같은 결론을 내는 자리가 늘었을 뿐이다.
//
...
describe("CLI-INTERFACE §1 — 축 3: 내장 모듈 허용 목록 폐쇄", () => {
  ...
  it("§1:20이 명시적으로 금지한 8종은 어떤 형태로도 나타나지 않는다", () => {
    // 위 단언에 포함되지만 따로 센다: §1:20의 금지 목록은 *왜* 금지인지가 절마다
    // 다르고(네트워크는 providers, 스폰은 tools, DB는 store의 본업), 이 이름들이
    // 들어오면 경계가 샌 것이지 목록이 낡은 것이 아니다.
    //
    // **이 배열은 §1:20의 금지 열거와 집합으로 같다** — 예산 게이트의 `cli` 항과도 같다.
    // `node:dgram`·`node:worker_threads`는 2026-08-24 개정에서 들어왔다: 워커는 자기
    // 컨텍스트에서 위 여섯을 그대로 열 수 있고 UDP 소켓은 TCP 금지가 안 덮는 출구라,
    // 둘을 함께 막지 않으면 «CLI는 직접 나가지 않는다»가 우회 가능한 상태로 남는다.
...
//
// **스코프 위반처럼 보이는 것에 대하여.** 이 테스트는 CLI 스코프인데 다른 10패키지의
...
/**
 * 워크스페이스 11패키지 전부 — fail-closed의 기대값이자 아래 넷 한정·여섯 분리의 존재 전제.
 *
 ...
 * 별도 스캔으로 모으면 fail-closed(11개 리터럴)가 한쪽만 덮어 F-3이 devDep 쪽에서
...
describe("CLI-INTERFACE §1 — 축 4: 유일한 조립 지점", () => {
  it("fail-closed: 수집된 워크스페이스가 11개 패키지 리터럴과 정확히 일치한다", () => {
  ...
  it("넷(core·tools·gate·store)의 형제 의존은 @neo-agent/core뿐이다 — §1:21이 기댄 전제", () => {
    ...
    //
    // 그래서 대상도 **그 넷으로 한정한다** — 이전에는 워크스페이스 전 패키지에 적용됐는데,
    // 그러면 넷이 아닌 나머지(정본이 각자 따로 있다 — 아래 분리 단언)의 위반까지
    // 이 단언이 죽으면서 진단이 무관한 네 문서로 독자를 보냈다(F-2). 넷의 존재는
    // 위 fail-closed의 11개 리터럴이 보증한다.
  ...
  it("나머지 여섯 패키지의 형제 목록이 각 정본 문서의 예산과 정확히 일치한다", () => {
    // 넷 한정에서 빠진 여섯의 정본은 CLI-INTERFACE도 위 넷의 §1도 아니라 **각자의
    ...
    const DOCUMENTED_SIBLINGS: Record<string, string[]> = {
      ...
      // `WEB-UI.md` §2.2 — `zod` + core 정확히 2개
      "@neo-agent/serve": ["@neo-agent/core"],
    };
