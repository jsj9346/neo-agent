...
// ───────────────────────────────────────────────────────────────────────────
// Q6. [미규정] 주입 싱크의 폭 — 판정 필요
// ───────────────────────────────────────────────────────────────────────────

describe("QA — 주입 싱크가 받는 것의 범위 (판정 필요)", () => {
  /**
   * **[미규정] `CliDeps.out`이 받는 것의 범위를 §1이 정하지 않았다.**
   *
   ...
   */
  it("[미규정] 주입 싱크가 조립 고지와 렌더러 출력을 함께 받는다", async () => {
   ...
   */
  it("[미규정] 종료 인사는 io로 나가고 주입 싱크로는 오지 않는다", async () => {
    const sink = captureSink();
    const rig = createRig({ deps: { out: sink } });
    app = await startCli(rig.deps, { kind: "run" });
