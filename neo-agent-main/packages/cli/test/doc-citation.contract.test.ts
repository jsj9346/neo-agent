...
describe("축 7 — D-5: 인용부호 구간을 바깥에서 정확히 감싼 강조 (§3.4)", () => {
  ...
  /*
   * **[미규정] 비대칭 강조 런이 D-5인가 — 열려 있는 것은 감쌈의 술어(D축)뿐이다.** 왼쪽과
   * 오른쪽 마커 개수가 다른 입력(별표 둘로 열고 하나로 닫는 형태 따위)을 D-7이 «마커 쌍»으로
   * 읽는지를 **정본이 가르지 않았다** — D-7은 쌍이 구간의 *"바로 바깥"*에서 열고 닫을 것만
   * 요구하고 두 런의 길이가 같아야 한다고는 하지 않으며, D-8은 부류만 든다. Q-1·Q-6이
   * 백틱·펜스에 대해 런 길이를 술어에 넣은 것과 달리 강조 런에는 그런 항이 없다.
   * **단언하면 정해지지 않은 것을 계약처럼 굳힌다.**
   *
   * **P축은 여기서 안 열려 있다.** 2026-08-16까지 이 주석은 *"D-5라면 열이 어디인가"*도 함께
   * 미규정으로 들었으나 §3.4 P-4가 그 값을 정했다 — *"발견의 열은 위반 형태의 첫 글자다"* ·
   * §4 *"D-5는 **감싼 강조 마커**의 첫 글자"*. 이 형태가 D-5로 판정되는 날 열은 P-4 첫째 절이
   * 정하므로, 남는 물음은 **D-5인가** 하나다. 위 두 표(폭 1·폭 2)가 P-4를 대칭 런에서 이미
   * 고정하고 있고 이 자리는 그 값을 다시 열지 않는다.
   */
...
describe("미규정 — 정본이 정하지 않은 자리 (§6)", () => {
   ...
   * 실단언으로 옮겨갔다.
   *
   * **이 목록에 남은 셋은 이번 사이클의 범위 밖이다** — 그 셋이 카드를 드는 시제가 오늘도
   * 맞는지는 여기서 판정하지 않고 `K-121`이 든다. **요약이 그 자리들의 카드 이름을 되풀이하지
   * 않는다**: 되풀이하면 그 카드가 세어 둔 자리 밖에 같은 포인터가 하나 더 생겨 **모집단이
   * 조용히 늘고**, 그 포인터가 닫힌 카드를 현재형으로 들면 이 목록이 다시 낡는다.
   */
});
