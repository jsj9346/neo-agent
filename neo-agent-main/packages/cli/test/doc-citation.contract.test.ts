...
describe("축 6 — 무엇이 코드이고 무엇이 인용부호인가 (§3.4 Q-1~Q-3)", () => {
  ...
  /*
   * [미규정] **여닫의 컨테이너가 어긋난 쌍** — 여는 줄만 `>` 접두가 붙고 닫는 줄은 안 붙은
   * (또는 그 반대) 형태. `.d.mts:85-87`이 *"남은 미규정은 여닫의 «컨테이너»가 어긋난 쌍이다"*로
   * 이 자리를 열어 두었고, §3.4는 여닫의 관계를 **인라인 스팬에 대해서만**(런 길이) 정하고
   * 펜스에는 들여쓰기도 컨테이너도 말하지 않는다. 두 읽기 다 비용이 있다 — 넓게 닫으면
   * 마스크가 문서 뒤로 번지고(침묵), 좁게 닫으면 정당한 펜스가 안 닫힌다. **구현이 낸 답을
   * 기대값으로 승격시키지 않는다.**
   */
  it.todo("여닫 펜스의 인용 블록 접두가 어긋난 쌍을 무엇으로 볼 것인가 — 판정 필요");
});
