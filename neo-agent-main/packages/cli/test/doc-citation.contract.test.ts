 ...
 *
 * **표본은 정본 문면에서 독립으로 도출했다** — 플랜 T-001이 든 수용 표본 목록을 베끼지 않는다.
 */
describe("축 8 — Q-7: 짝이 어긋난 입력에서 조용한 0을 내지 않는다 (§3.4 · §4)", () => {
  ...
  it("문서 전체로는 짝수여도 문단마다 홀수면 발견이다 — 상한은 문단 경계다", () => {
    ...
    expect(names(findings)).toContain(UNPAIRED_QUOTE);
    // 자리는 **그 문단의 첫 글자**이므로 문단이 시작하는 줄만 나온다. [미규정] 발견이 문단마다
    // 하나인지 문서마다 하나인지는 §3.4가 정하지 않는다 — 수를 단언하지 않는다.
  ...
  it("홀수 갈래의 자리는 그 문단의 첫 줄이다 (1-기반)", () => {
    const doc = [
      "첫 문단은 짝이 맞는다.",
      "",
      '둘째 문단이 "를 하나 들고',
    ...
    // [미규정] 열의 기준점 — §3.4는 *"그 문단의 첫 글자"*라 적었으나 §4·`.d.mts`는 «1-기반»만
    // 든다. 값을 굳히면 정해지지 않은 것을 계약으로 만든다(아래 미규정 describe의 «D-5 열의
    // 기준점» 항과 같은 자리).
    expect(finding.column).toBeGreaterThanOrEqual(1);
  });
...
describe("미규정 — 정본이 정하지 않은 자리 (§6)", () => {
  ...
  /* --- 2026-08-16 Q-7이 새로 연 자리 (축 8) ----------------------------------- */

  /**
   * §3.4는 자리를 *"그 문단의 첫 글자"*로 정했으나 **발견의 수**를 정하지 않는다 — 홀수인 문단이
   * 둘이면 발견도 둘인가 그 문서에 하나인가. 규칙 칸의 결론이 *"그 **문서**의 인용부호 구간은
   * 판정되지 않은 것이다"*로 문서 단위이고, 자리 문단은 문단 단위라 두 읽기가 다 선다. 축 8은
   * 그래서 수를 단언하지 않는다. **단언하면 정해지지 않은 것을 계약처럼 굳힌다.**
   */
  it.todo("홀수 문단이 둘이면 Q-7 발견도 둘인가 문서에 하나인가 — 판정 필요");

  /**
   * §3.4가 Q-7의 자리를 *"그 문단의 첫 글자"*로 든 것은 **열의 기준점을 정한 것으로도 읽힌다**
   * (문단의 첫 글자 = 그 줄의 첫 칸). 그러나 §4는 D-5와 «같은 형태»라고만 하고, 위 «D-5 열의
   * 기준점» 항이 그 형태의 열을 아직 안 정했다. 두 항이 같은 자리를 반대편에서 물고 있다.
   * **미닫힘 갈래는 자리가 더 갈린다** — 2026-08-16 관측에서 열이 여는 줄의 **마커 시작**을
   * 가리켰고(들여쓴 펜스·인용 블록 접두에서 1이 아니다), §3.4는 «여는 줄»까지만 든다.
   */
  it.todo("Q-7 홀수 갈래의 열은 문단 첫 글자로 고정되는가 — 판정 필요");
});
