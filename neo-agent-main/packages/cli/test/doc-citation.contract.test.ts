...
describe("축 6 — 무엇이 코드이고 무엇이 인용부호인가 (§3.4 Q-1~Q-6 · P-1·P-2·P-3·P-6)", () => {
   ...
   *
   * **2026-08-16 — 구간의 경계를 함께 단언한다(P-1).** 이 자리는 그날까지 미규정이라
  ...
  it.each([
    ...
    (_label, source, span텍스트) => {
      const span = head(quoteSpans(source), "인용부호 구간이 잡히지 않았다");
      // 2026-08-16 — 이 자리의 미규정 판정이 §3.4 P-1로 닫혔다(*"인용부호 글자도 … 구간 안이다"*).
   ...
   *
   * 이 자리는 2026-08-15까지 미규정이었다 — 그때 `scripts/doc-citation.d.mts`의 `FENCE_LINE`
...
/**
 * 선언 파일에서 주석을 벗긴다. 글자를 지우지 않고 **공백으로 덮는다** — 아래 술어가 전부
 * `^`와 `m` 앵커에 기대므로 줄과 열이 밀리면 앵커가 어긋난다.
 *
 * **앵커를 더 조이는 것으로는 안 닫힌다.** 블록 주석 안에서 열 0으로 시작하는 줄이 그 앵커를
 * 그대로 만족한다 — 주석에 적힌 선언 한 줄이 분류의 모집단에 들어오고, 유니온 별칭을 찾는
 * 첫 매치를 가로챈다. 그래서 실질 수단은 마스킹 하나다.
 *
 * **파일-로컬인 이유.** 이 축이 재는 대상이 `scripts/comment-lexer.mjs`의 표면 자신이라,
 * 그 모듈의 소속 술어를 여기서 빌리면 재는 것과 재이는 것이 같아진다 — 렉서가 망가지는 날
 * 이 축은 red가 아니라 조용한 green을 낸다. 게다가 그 임포트는 동적이라 모듈 스코프에 선
 * 아래 상수들을 세우지 못한다.
 *
 * **문자열은 남긴다.** 아래 `unionMembers`에게는 문자열 리터럴이 곧 모집단이라, 문자열까지
 * 지우는 수단을 걸면 그 술어가 상시 붉어진다.
 *
 * **한계** — 문자열 리터럴 안에 여는 표기가 있으면 주석으로 오인한다. 대상이 이 선언 파일
 * 하나뿐이고 오늘 그런 리터럴이 없어 안 걸린다. 걸리는 날 실물 선언이 함께 덮여 패리티가
 * 갈리므로, 조용한 통과가 아니라 red로 드러난다.
 */
...
/**
 * 계약면 — 이 축이 여는 유일한 실물이다. **주석을 벗긴 뒤에 잰다**: 이 축이 재는 표면은
 * 선언이지 그 선언을 설명하는 주석이 아니다. 이 한 자리가 아래 다섯 술어 전부의 상류라,
 * 여기서 벗기면 정규식을 지점마다 복제하지 않아도 된다.
 */
const LEXER_DECLARATION = stripComments(
  readFileSync(
