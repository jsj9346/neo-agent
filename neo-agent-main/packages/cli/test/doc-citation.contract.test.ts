 ...
 * 둘 다 P-6과 같은 사이클에 개정되는 중이라 «선언이 이러니 이게 맞겠지»가 성립하지 않는다.
 */
...
describe("축 6 — 무엇이 코드이고 무엇이 인용부호인가 (§3.4 Q-1~Q-6 · P-1·P-2·P-3·P-6)", () => {
   ...
   *
   * **역방향은 이 파일에 이미 있다 — 새로 세지 않는다.** 축 7의 «`*"…"*`는 인용부호 형식이지
   * 바깥 강조가 아니다»가 그 자리이고(D-5 0건), 위 두 방향은 같은 값의 앞뒤다. 여기서는
   * **구간의 경계**만 잰다.
   * ------------------------------------------------------------------------ */
   ...
   * 고정하지 않았으므로 문자열을 굳히면 계약에 없는 것을 계약으로 만든다»였고, 계약이 실제로
...
describe("미규정 — 정본이 정하지 않은 자리 (§6)", () => {
   ...
   * 이 자리는 그 물음을 `it.todo`로 들어 «아직 안 정해졌다»를 판정이 난 뒤에도 서술했다.
   ...
   *
   * **같은 날 2026-08-15 이관분이 열었던 자리 둘도 닫혔다.** «`QuoteSpan`은 인용부호 문자를
   * 포함하는가»는 P-1이(*"인용부호 글자도, 첫 형식의 바깥 별표도 구간 안이다"*),
