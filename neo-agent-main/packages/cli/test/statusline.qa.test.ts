...
describe("§7.1 ① 구별과 비침묵 — 그려진 상태줄", () => {
   ...
   * 무엇을 심어도 통과하는 단언이다.
   *
   * 같은 테스트가 §7.1의 «TTY가 아니면 그리지 않는다»를 함께 잰다.
   */
...
describe("§7.1 ③ 걷은 행 수와 그린 행 수", () => {
  ...
  /**
   * [미규정 SL-1] 그리는 시점과 걷는 시점 사이에 **폭 판정이 갈리는** 경우 — 현재 동작 기록.
   *
   ...
   * 갈리고, 그리면 §7.1이 금지한 결과 — 걷히지 않은 상태줄이 트랜스크립트 안에 죽은
   * 채로 박히는 것 — 가 나온다. 아래가 그 관측이다.
   *
   * **판정하지 않는다.** §7.1은 세션 중 폭 변화(리사이즈·폭 상실)를 다루지 않고,
   * 단위성 조항의 근거 문장은 «두 영역에 각자의 클리어 경로»를 겨눈다 — 구현은 경로가
   * 하나이므로 그 문면에는 걸리지 않는다. 결과 쪽으로 읽으면 걸린다. 어느 독해가
   * 맞는지는 보고서가 판정을 요청한다.
   *
   * 이 단언은 **지금 동작을 기록**한다. 판정이 나서 고쳐지면 여기가 붉을 것이고,
   * 그때 이 주석과 보고서를 함께 읽고 단언을 뒤집으면 된다.
   */
  it("SL-1 — 폭을 잃으면 걷히지 않은 상태줄이 트랜스크립트에 박힌다 — 현재 동작 기록", () => {
  ...
  /**
   * [미규정 SL-2] 감싸인 입력 라인에서 커서를 앞 행으로 올린 뒤 제출하는 경로.
   *
   * **이것을 정할 정본은 `docs/CLI-INTERFACE.md` §7.1이다** — 말미의 측정 셋 중 셋째가
   * 이 경로를 범위에 두는가.
   *
   * readline이 감싸인 라인을 자기 방식으로 되그리는 자리라 걷은 행 수와 실제로 그려진
   * 행 수가 어긋난다 — **우리 개입이 없어도 그렇다**는 것이 `src/input.ts`의 R-1 실측
   * 3이 든 선재 결함이고, §7.1은 그 결함을 우리가 **새로 만드는 것**을 금지한다.
   * 그래서 여기서 재는 것은 **어긋나는가**가 아니라 **§7.1이 상태줄을 들이면서 그 결함을
   * 넓혔는가**다: 상태줄이 있는 화면과 없는 화면에서 남는 행 수가 같아야 한다.
   *
   * 어느 쪽을 계약으로 볼지는 정본이 정하지 않았다 — §7.1 말미의 측정 셋은 «걷은 행
   * 수와 그린 행 수가 같은가»를 조건 없이 들고, 그 조건 없는 문면대로 읽으면 이 경로는
   * 위반이다. 판정은 보고서가 요청한다.
   */
...
describe("§7.1 내용 표 — 세션 교체 뒤에도 갱신이 이어진다", () => {
  it("/new 뒤의 턴에도 상태줄 사용량이 갱신되고, 이전 세션 값은 남지 않는다", async () => {
    ...
    /**
     * [미규정 SL-3] 세션이 바뀐 **직후**의 사용량 항목.
     *
     * **이것을 정할 정본은 `docs/CLI-INTERFACE.md` §7.1이다** — 내용 표의 갱신 계기 열.
     *
     * 세션 id는 여기서 곧바로 갈린다(§7.1 표의 갱신 계기 열이 `/new`·`/resume`을 든다).
     * 사용량의 갱신 계기는 `turn_end`뿐이라, 새 세션의 첫 턴이 끝나기 전까지 상태줄에
     * 실려 있는 수는 **방금 떠난 세션의 것**이다. 정본은 세션 교체 때 이 항목을
     * 비울지 말지를 정하지 않았다 — 사용량은 §7.1이 세부로 분류한 셋 중 하나이므로
     * 계약 위반으로 판정하지 않고, 보고서가 판정을 요청한다.
     *
     * 아래는 그 관측을 기록만 한다. 어느 쪽으로도 단언하지 않는다 — 지금 동작을 굳히면
     * 세부가 계약이 되고, 반대 단언은 정본에 근거가 없다.
     */
