...
/**
 * `0`의 존재 검사를 지난 홈으로 만든다 — 판정 재료는 `sessions.db`의 부재 하나뿐이라
...
describe("WEB-UI §3 — serve 기동·왕복·종료", () => {
  ...
  it("S-4 sessions.db가 없으면 비영으로 끝나고 열린 포트가 0이다", async () => {
    // 홈은 있고 DB만 없다 — `0`의 판정 재료가 DB의 부재 하나뿐이라는 것의 실물이다.
    const rig = createRig();
  ...
  it("S-8 runCli의 serve 갈래가 §3.1 거부까지 닿고 REPL을 열지 않는다", async () => {
    // DB가 없는 홈으로 `runCli(["serve"])`를 부른다. 조립이 `0`에서 던지고 그 문면이
   ...
   *
   * **위 S-8이 이 축을 대신하지 못한다.** 그 기동은 `0`에서 던지므로 라우팅이 새도 같은
