 ...
 * **행동**을 잰다: 탭이 completer에 닿는가. 가드가 자기 readline을 따로 세우므로
 * **환경발 강등은 경로를 묻지 않고 걸린다** — `TERM` 말고 다른 것이 같은 강등을
 * 들여와도 마찬가지다.
 *
 * **덮지 않는 갈래 하나.** 강등이 환경이 아니라 `createRepl` 쪽에서 들어오면(예:
 * `terminal: true`가 빠지면) 가드는 그린인 채 계약 축들이 빨개진다. **그게 맞다** —
 * 그때 깨진 것은 측정 장치가 아니라 제품이므로 red가 정답이다. 즉 「가드 그린인데
 * red」를 계약 위반으로 읽어도 되지만, 그 근거는 이 가드가 아니라 `createRepl`의
 * 옵션이 계약 표면이라는 별개 사실이다(2026-09-09 독립 QA 실측 — F-2).
 *
 ...
 *
 * **그 행 자체는 이 파일에도 있다** — 아래 `approval-wait` 행의 `it`이 EOF가 REPL에
 * **도달하지 않는다는 구조**를 재고, 그것은 처분과 다른 대상이다. 이 구절이 없으면 머리가
 * 「그 행은 여기 없다」로 읽혀 그 `it`을 못 찾게 한다.
 *
 * **지목을 서수로 하지 않는다.** 이 구절은 「아래 마지막 `it`」이었고 그것이 이미 틀려 있었다
 * (2026-09-09 — 이 절의 `it`은 다섯이고 그 구조를 재는 것은 넷째다. 마지막은 `close()` 테스트라
 * EOF를 아예 안 다룬다). 서수는 `it`이 하나 덧붙으면 조용히 틀어지므로 줄번호와 같은 부패
 * 계열이고, 처방도 같다 — 이 파일 머리가 문서 지목에 대해 고른 것처럼 **이름으로 가리킨다.**
 */
describe("종료", () => {
