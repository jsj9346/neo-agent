 ...
 * (2026-09-09 관측 — 그때 이 절의 `it`은 다섯이었고, 그 구조를 재는 것은 넷째, 마지막은
 * `close()` 테스트라 EOF를 아예 안 다뤘다). 서수는 `it`이 하나 덧붙으면 조용히 틀어지므로
 * 줄번호와 같은 부패 계열이고, 처방도 같다 — 이 파일 머리가 문서 지목에 대해 고른 것처럼
 * **이름으로 가리킨다.**
 */
describe("종료", () => {
