...
describe.skipIf(!hasPty)("첫 기동 관문 — 실 pty 종료 축 (CLI-INTERFACE §2.1)", () => {
   ...
   *
   * **[미규정 — 판정 필요] 「확인」의 경계를 정본이 정하지 않았다.** 여기서는 키 입력 뒤에
   * 나온 출력으로 읽는다 — 관문 문면 자체가 두 이름을 함께 들므로 화면 전체로 재면 둘째
   * 단정이 원리적으로 설 수 없고, 이 독해는 그 절이 확인을 *"선택 뒤의"* 표시로 부른 것에서
   * 나온다. **관문을 통째로 다시 그리고 그 아래에 확인을 붙이는 구현**은 이 독해에서 붉고
   * 다른 독해에서는 초록이다. 정본이 그 갈래를 안 가른다 — 임의로 판정하지 않는다.
   */
  it(
...
describe.skipIf(!hasPty)("첫 기동 관문 — 계속(Red Pill) 갈래 (CLI-INTERFACE §2.1·§2)", () => {
   ...
   *
   * **[미규정 — 판정 필요] ⓔ와 같은 미규정이 이 자리에도 그대로 걸린다.** 「확인」의 경계를
   * 정본이 정하지 않았고, 여기서는 키 입력 뒤의 출력으로 읽는다. 계속 갈래에서는 그 뒤에
   * 시작 화면(§2의 5b·6 고지)이 이어지므로 이 구간이 확인보다 넓다 — 넓은 쪽으로 재는 것이
   * 둘째 단정을 **더 엄하게** 만들므로(안 고른 이름이 그 어디에도 없어야 한다) 이 독해가
   * 이 축을 느슨하게 하지는 않는다. 그래도 경계는 여전히 미규정이다.
   */
  it(
    "ⓖ 확인이 비어 있지 않고 고르지 않은 쪽의 이름을 담지 않는다",
    async () => {
   ...
   * 시퀀스를 실제로 돌린다(*"세션 id와 재개 방법 ... 을 표시하고 종료한다"*).
   *
