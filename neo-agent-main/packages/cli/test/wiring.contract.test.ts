 ...
 *
 * 아래 넷은 전부 **2026-08-06에 판정됐다.** 판정 정본은
 ...
 *   `integration.contract.test.ts`의 I-2와 **같은 항목**이다. `src/renderer.ts`는
 *   `renderTranscript`를 내보내지만 `src/index.ts`는 재수출하지 않는다.
 *   **판정 — 내부 전용 확정** (2026-08-06). §6의 이행 세부이고 소비자는 wiring
 *   하나(같은 패키지)이므로 배럴에 올리지 않는다. 배럴에 없는 것이 계약 위반이
 *   아니라 **계약의 결론**이다.
 * ═══════════════════════════════════════════════════════════════════════════ */
