...
describe("배선 — 설정 동결 (SAFE-DEFAULTS §4)", () => {
   ...
   *
   * 같은 물음이 `test/harness.ts`의 QA-A 블록(U-6)에도 **마커인 채 남아 있다** —
   * 같은 판정으로 닫히지만 이번 사이클의 범위 밖이었다. 한쪽만 보고 열린 것으로
   * 읽지 않도록 여기 적어 둔다(2026-08-11 독립 QA 발견).
   */
 ...
 *
 * W-1~W-3은 **2026-08-06에 판정됐다.** 판정 정본은
 * `plans/20260806-cli-qa-report.md` — W-1은 §3(계약 명문화 표), 나머지는
 * §4(조치 불필요 15건)다. 열린 판정이 아니므로 미규정 마커를 달지 않는다 —
 * **닫힌 항목이 마커를 유지하면 진짜 열린 항목이 묻힌다.**
 *
 * **W-4는 예외로 열려 있다** — 2026-08-11 독립 QA가 그 판정의 전제가 거짓임을
 * 실측했다(아래). 닫힌 것을 닫고 열린 것을 남기는 것이 이 블록의 요점이다.
 *
 ...
 *
 * ── [미규정] W-4: `renderTranscript`의 배럴 노출 — **닫히지 않았다** ────────
 *   `integration.contract.test.ts`의 I-2와 **같은 항목**이다.
 *   2026-08-06 판정은 "내부 전용 확정 — §6의 이행 세부이고 소비자는 wiring
 *   하나(같은 패키지)"였다(`plans/20260806-cli-qa-report.md` §4). 그러나
 *   **2026-08-11 독립 QA 실측: `src/index.ts`가 `renderTranscript`를 실제로
 *   재수출한다.** 그 판정이 전제한 "배럴에 없다"는 판정 시점에 이미 거짓이었다 —
 *   같은 판정 대상 사슬의 마지막 커밋(`5c21c0a`)에서 배럴에 올라갔기 때문이다.
 *   즉 이 항목의 물음은 그대로 열려 있고, 답은 둘 중 하나여야 한다:
 *   (a) 판정대로 내부 전용이면 **배럴에서 내린다**, (b) 배럴 노출이 맞으면
 *   **판정을 갱신한다**(§6의 "렌더러가 직접 그린다"의 공개 표면으로).
 *   어느 쪽이든 계약 문서와 실물이 함께 움직여야 하므로 여기서 정하지 않는다.
 *   계약 위반으로 단언하지 않고 판정만 요청하는 것은 원 마커와 같다.
