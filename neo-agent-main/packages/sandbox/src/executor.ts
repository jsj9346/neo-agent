...
export function createDockerShellExecutor(options: DockerShellExecutorOptions): {
  ...
  function buildRunArgs(containerName: string, runId: string, request: ShellExecRequest): string[] {
    const args = [
      ...
      // **[미규정 ES-9]** 옵션 문자열(`nosuid,nodev,mode=1777`)과 **크기 상한 미지정**은
      // 문서가 정하지 않았다. 크기를 안 건 이유는 tmpfs 페이지가 컨테이너 메모리
      // cgroup에 잡히므로 `--memory`가 이미 상한이기 때문이다 — 여기서 별도 수치를
      // 정하면 상한이 둘이 되고, 작은 쪽이 조용한 실패를 만든다
      "--tmpfs",
