...
/**
 * 판정 상한 기본값. 계약은 여전히 "상한이 존재한다"이지 수치가 아니며(`SANDBOX.md`
 * §3) 이 값은 조정 가능한 세부다.
 *
 * **실측으로 정할 값이 아니다.** 재서 답이 나오는 것은 컨테이너 자원 수치(§8 — 상한이
 * 실제로 무는가) 쪽이고, 이쪽은 오차 방향의 선택이다: 짧게 잡으면 **느리지만 살아 있는
 * 데몬**을 불가용으로 오판해 셸 도구를 통째로 숨기고, 길 때의 비용은 **이미 고장 난
 * Docker에서만** 난다. 근거 전문은 §3이 갖는다 — 여기 옮겨 적지 않는다.
 */
