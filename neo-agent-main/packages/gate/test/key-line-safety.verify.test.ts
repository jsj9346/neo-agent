 ...
 *
 * §10은 개행이 들어갈 수 있는 자리를 **둘**로 든다(셸 명령 · `cwd`). 이 파일은 그
 * 열거의 완전성을 잰다 — 파일 도구의 경로도 모델이 제어하는 문자열이고 POSIX 경로는
 * 개행을 가질 수 있다.
 *
...
describe("L-1 학습 키는 개행을 갖지 않는다 (CLI-INTERFACE §10 — 「한 줄」의 주체는 게이트)", () => {
  ...
  it("파일 도구 경로: 개행이 든 경로에는 키를 주지 않는다", async () => {
    // §10이 열거하지 않은 셋째 자리다. POSIX 경로는 개행을 가질 수 있고
    // `WorkspaceBoundary.resolve`가 그대로 돌려준다.
