...
describe("L-4 가드의 정의역은 원시함수가 정한다 (CLI-INTERFACE §10 — 열거하지 않는다)", () => {
  ...
  // 검사」로 부르는 형태다.
  //
  // **이 규율은 이웃 `cwdBreaksKeySyntax`에도 그대로 걸린다**(2026-09-09 · `K-609`로
  // 갱신). 그전엔 여기가 「그 함수는 판정이 `:`·개행 둘뿐이고 파이프라인 위에서 전부
  // 관측되므로 단위 단언을 갖지 않는다」로 적혀 있었는데, 같은 개정이 그 함수에
  // **트림 축을 셋째로 더해**(APPROVAL-GATE §4 — 근거는 왕복이 아니라 화면 단사성이다)
  // 두 절이 다 거짓이 됐다. **그리고 트림 축은 파이프라인 위에서 가려진다** — 변이
  // 시험이 그것을 든다(2026-09-09 실측 2회, T-004와 T-006이 각각): 그 함수에서
  // `cwd !== cwd.trim()` 항만 지우면 레포 전량 4906개 중 붉어지는 것은 **단위 단언
  // 하나뿐**이고, 파이프라인 층은 전부 초록으로 남는다. 그래서 그 함수도 「가려지는
  // 정의역이 있는 쪽」이 되어 단위로 내려왔고, 그 자리는
  // `packages/gate/test/key-line-safety-qa.contract.test.ts`의 `C-6`이다.
