 ...
 * 나타내려면 `cwd` 필드가 `:`을 갖지 않아야 한다. 아니면 `cwd`가 `/ws:a`인 `npm test`와
 * `cwd`가 `/ws`인 `a:npm test`가 **둘 다 `shell:/ws:a:npm test`**가 되어 한쪽 승인이
 * 다른 쪽을 통과시킨다 — 키를 좁히러 온 개정이 새 넓힘을 심는 자리다. 보장이 서면
 * 첫 `:`이 언제나 경계이므로 명령 쪽은 `:`을 자유롭게 가져도 된다.
 *
 * **충돌하려면 `:`이 `cwd` 안에 있어야 한다.** 이 주석의 초판은 `(/ws/a, "npm test")`와
 * `(/ws, "a:npm test")`를 예로 들었는데 그 둘은 `shell:/ws/a:npm test`와
 * `shell:/ws:a:npm test`로 실제로는 갈린다 — 규칙은 옳고 예시가 틀렸다
 * (2026-09-09 정정, 독립 QA가 잡았다).
 *
 ...
 *
 * **트림 축은 근거가 이웃과 다르다 — 왕복이 아니라 화면 단사성이다**
 * (2026-09-09 추가 · `K-609`). 파일 도구 경로의 트림 축(`pathBreaksKeySyntax`)이 서는
 * 근거는 **왕복**이다 — 그 경로는 키의 꼬리라 양끝 공백이 파일의 트림 읽기에 먹혀
 * `add`한 키와 `has`가 묻는 키가 갈린다. `cwd`는 그렇지 않다: `shell:`과 정규화된 명령
 * **사이**, 즉 트림이 닿지 않는 가운데에 있어 키 전체의 왕복은 안 깨진다. 그래서 이웃의
 * 근거는 여기 그대로 오지 않는다.
 *
 * 그럼에도 막는 근거는 **다른 것**이다: `작업 디렉터리: /ws/x `와 `작업 디렉터리: /ws/x`는
 * **화면에서 같은데 키가 갈린다.** 사용자가 같은 화면을 두 번 보고 승인했는데 학습이
 * 한쪽에만 서면 두 번째 프롬프트는 **이유를 알 수 없는 마찰**이 되고, 그것이 §2 계층 4b의
 * 경고 문면이 존재하는 바로 그 근거다(이유를 모르는 마찰은 승인 피로를 거쳐 무조건
 * allow로 간다). 「승인 UI가 거짓말하지 않는다」의 **키 쪽 얼굴**이다.
 *
 * **모집단은 비어 있지 않다.** `WorkspaceBoundary.resolve`는 존재하지 않는 말단 성분을
 * 가장 가까운 실재 조상의 realpath에 **그대로 이어 붙이므로**(`packages/tools/src/workspace.ts`)
 * 양끝 공백은 살아남는다 — 경로 판정기가 realpath를 돌려준다는 이유로 이 축을 비워 두면
 * 안 된다(APPROVAL-GATE §4가 독립 QA의 U-1을 그 근거로 뒤집었다).
 *
 * **이 축은 명령 쪽으로 넓히지 않는다.** 명령은 `canonical`로 접혀 키에 들어가고 그
 * 정규화가 이미 양끝을 트림하므로 화면에서 같아 보이는 두 명령은 같은 키가 된다 — 물음
 * 자체가 성립하지 않는다. 정의역은 **키에 원문 그대로 들어가는 문자열**, 즉 `cwd`와 파일
 * 도구 경로 둘뿐이다. 걸면 모델이 흘린 양끝 공백 하나로 명령이 영영 학습 불가가 된다.
 *
 * **자리 수는 늘지 않는다.** 축을 하나 더하는 것이지 §4의 「다섯 번째 사용」이 여섯이
 * 되는 것이 아니다 — 이 판정기는 이미 서 있는 **네 번째 자리**이고 그 자리가 축을 하나 더
 * 갖는 것뿐이다(계약 테스트 `C-5`가 그 다섯을 한 표에서 잰다).
 *
 * **판정을 문자 열거로 쓰지 않고 `String.prototype.trim`에 위임한다** — 이웃
 * `pathBreaksKeySyntax`와 같은 규율이고 근거도 같다: `trim`이 걷는 집합은 ASCII 공백 셋이
 * 아니라 NBSP·전각 공백·U+2028·U+FEFF까지라, 열거로 옮겨 적으면 목록이 실물보다 좁아지는
 * 부패가 다시 생긴다.
 *
...
export function cwdBreaksKeySyntax(cwd: string): boolean {
  return /[:\n\r]/.test(cwd) || cwd !== cwd.trim();
}

/**
 * allowlist 학습 불가 판정 — 키 문법을 깨뜨리는 파일 도구 경로
 * (APPROVAL-GATE §4, 2026-09-09).
 *
 * 파일 키는 `<kind>:<해석된 절대 경로>`이고, 근거는 이웃 `cwdBreaksKeySyntax`와 다르다 —
 * **단사성이 아니라 파일 포맷과 왕복이다.** 첫 `:`이 언제나 경계이고 그 뒤가 전부 경로라
 * 단사성은 이미 서므로 `:`은 보지 않는다. 같은 금지를 경로에 걸면 `:`을 가진 정상 경로가
 * 이유 없이 학습 불가가 된다 — `cwd`가 `:`을 막는 것은 뒤에 필드가 하나 더 오기
 * 때문이고, 그 비대칭은 의도된 것이다.
 *
 * 두 축이 각각 선다:
 *
 * - **한 줄 1키.** allowlist는 한 줄이 키 하나이고(`CLI-INTERFACE.md` §10) 파일 구현은
 *   받은 키를 그대로 한 줄로 쓴다 — 이스케이프를 시작하면 읽는 쪽에 대응 디코더가
 *   필요해지고 키 형식의 정본이 둘이 된다. POSIX 경로는 개행을 가질 수 있고 경로
 *   판정기가 그것을 그대로 돌려주므로, 개행이 든 키는 파일에서 두 줄이 되고 **뒤 줄이
 *   그 자체로 유효한 키 모양**이라 사용자가 승인한 적 없는 대상이 영구 학습된다.
 *   개행을 따로 검사하는 것은 **가운데** 개행을 트림이 걷지 못하기 때문이다.
 * - **트림 불변.** 파일 구현은 줄을 트림해 읽는다 — 손편집 관용이고, 게이트가 파일을
 *   재읽기하지 않아 손편집이 학습을 회수하는 유일한 수단이라 그 관용이 계약이다.
 *   그래서 게이트는 트림에 불변인 키만 내야 한다. 양끝이 공백인 키를 발급하면 `add`는
 *   성공하는데 다음 세션의 `has`가 거짓이 되고, 중복 차단이 갈려 같은 키가 세션마다
 *   한 줄씩 쌓인다.
 *
 * **판정을 문자 열거로 쓰지 않고 `String.prototype.trim`에 위임한다.** 계약이 그
 * 원시함수의 이름으로 진술돼 있고(`CLI-INTERFACE.md` §10), `trim`이 걷는 집합은 ASCII
 * 공백 셋이 아니라 NBSP·전각 공백·U+2028·U+FEFF까지다 — 열거로 옮겨 적으면 목록이
 * 실물보다 좁아지는 부패가 다시 생긴다. 위임 자체는 파이프라인 위에서 관측되지 않으므로
 * (트림 문자 대부분을 계층 4의 위조 탐지가 먼저 삼킨다) 단위 단언이 함께 선다.
 *
 * NUL은 검사하지 않는다 — POSIX 경로가 가질 수 없다.
 *
