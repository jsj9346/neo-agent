 ...
 * - **표가 앵커 목록 전부를 판정하는가 — 2026-09-02부로 잰다**(§9.6 결정 15). 그 전까지 이
 *   항목은 여기에 「안 잰다」로 서 있었다: §9.6이 범위 표에 그 전수성을 요구하지 않았고 결정
 *   11이 든 것은 부트가 여는 이름의 집합과 표의 일치뿐이었으므로, 앵커가 하나 늘었는데 표가
 *   안 따라와 그 이름이 「붙는다」로도 「안 붙는다」로도 안 읽히는 것을 여기서 붉히면 이 파일이
 *   정본에 없는 계약을 새로 짓는 것이 됐다. 그래서 `[미규정]`을 달고 판정을 §9.6에 넘겼다.
 *   **그 판정이 났다** — 결정 15 셋째 항이 범위 표를 *"닫힌 표가 아니라 전수 대조 대상이다"*로
 *   세우고 `anchors.js`의 `ANCHOR_NAMES`와 표의 행이 1:1일 것을 요구한다(*"표에 없는 이름이
 *   실물에 생기면 그 이름은 판정 없이 조용히 범위 밖으로 빠진다"*). 그 대조는 아래 축 1의
 *   마지막 테스트가 든다 — 이 항목은 더 이상 «재지 못하는 것»이 아니다.
...
describe("축 1 — 배선하는 앵커가 범위 표 그대로다 (§9.6 결정 11)", () => {
  ...
  test("표가 앵커 목록 전부를 판정한다 — 판정 없는 이름이 없다", () => {
    // 결정 15 셋째 항 — 범위 표는 *"닫힌 표가 아니라 전수 대조 대상이다"*이고, `anchors.js`의
    // `ANCHOR_NAMES`(닫힌 집합)와 표의 행이 1:1이어야 한다. 위 부분집합 축은 「표 밖 이름이
    // 배선됐다」는 방향만 잡고 반대 방향이 조용하다 — *"표에 없는 이름이 실물에 생기면 그 이름은 판정
    // 없이 조용히 범위 밖으로 빠진다"*. 그 순간 §9.6이 안 재기로 한 것과 빠뜨린 것이 구별되지
    // 않는다(§2.3). 이 축이 두 방향을 함께 닫는다.
    const judged = [...SCOPED_ANCHORS, ...OUT_OF_SCOPE_ANCHORS].sort();
