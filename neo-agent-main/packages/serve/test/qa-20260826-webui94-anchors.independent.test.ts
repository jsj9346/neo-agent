 ...
 *
 * **[처분됨 — K-316 · 2026-08-27] 「배선 0건」이 기대는 낱말의 뜻을 여기 적는다.** 위 문단의
 * 판정(모집단이 공집합)은 유지된다 — 바뀌는 것은 **무엇을 세어 그 말을 하는가**다. 결정 7이 든
 * 배선은 화면이 준 자리를 채우는 층이고 그것은 오늘 0건이다. 그러나 **조회 통로 자체는 이미
 * 섰고**(`client/wiring.js`) 그 통로가 DOM 조회 API를 실제로 부른다 — 인자가 변수라 아래
 * 스캐너가 그것을 안 볼 뿐이다. 그래서 아래 실물 축이 단언하는 것은 「DOM 조회 0건」이 아니라
 * **「인용부호 리터럴로 적힌 DOM 조회 0건」**이고, 그 좁힘 없이 적으면 §2.3이 이름 붙인
 * 형태(검사가 실제보다 넓게 주장하는 것)를 이 축이 저지른다.
 * ========================================================================= */
