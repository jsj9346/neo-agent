/**
 * 독립 QA — `docs/WEB-UI.md` §2.3 · §9.4(결정 5·7·8·9·10·12·13) · §6.1(`StateSnapshot` 절) ·
 * §9.2 · §9.3 · §9.5 결정 4 · §9.6 결정 12 · `docs/CLI-INTERFACE.md` §7.1 그리고
 * `docs/SAFE-DEFAULTS.md` §1·§2(축 3 리터럴 기대값의 정본).
 *
 * **이 열거의 정의역**: 축이 **기대값을 뽑은** 자리와 축이 **자기 한계의 근거로 드는** 자리다
 * (뒤엣것이 §2.3이다). **문서를 미리 정해 닫지 않는다 — 정의역이 문서를 고른다.** 반대로
 * **서술의 곁근거로만 부르는 자리는 안 든다**: 의존성 예산 §2.2 · 기동 §3.1 · 침묵 실패의
 * 이름 `ARCHITECTURE.md` §2.6 · 마커 규약 `MARKERS.md` §3.2·§4.2 — 본문이 자리마다 이름으로
 * 부른다. 정의역을 안 적으면 이 열거가 인용 색인으로 자라거나 다시 좁아진다.
 *
 * [처분됨 — K-361 · 2026-08-28] 이 열거는 §9.4 결정 7·8·9·10과 결정 12만 들고, 본문이 실제로
 * 대조 상대로 드는 §9.4 결정 13(축 1) · 결정 5(축 2의 하한) · §9.5 결정 4 · §9.6 결정 12를
 * 빠뜨리고 있었다. **반대 방향으로도 틀렸다** — 열거가 들던 §9.1은 본문에 한 번도 안 나오는
 * 죽은 항이라 걷었다. 그리고 **빠진 자리가 둘 더 있었다**: 본문이 네 자리에서 축 설계의
 * 근거로 드는 §2.3과, 축 3이 리터럴 기대값의 정본으로 명시 인용하는 `SAFE-DEFAULTS.md`
 * §1·§2다. 뒤엣것은 **문서를 둘로 닫는다고 적은 정의역 문장 자신이 가린 자리**이고, 그래서
 * 이 사이클이 그 문장을 함께 고쳤다. **규정은 처음부터 지켜졌고 틀린 것은 열거의 완전성이다**
 * — `CLI-INTERFACE.md` §7.1이 2026-08-21에 자기 열거에 대해 같은 판정을 쓴 형태 그대로다.
 *
