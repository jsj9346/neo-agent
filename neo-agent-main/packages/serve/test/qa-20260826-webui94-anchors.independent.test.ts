 ...
 *
 * **이 열거의 정의역**: 축이 **기대값을 뽑은** 자리와 축이 **자기 한계의 근거로 드는** 자리이고
 * (뒤엣것이 §2.3이다), 문서는 위 둘로 닫는다 — 다른 문서(`ARCHITECTURE.md` §2.6 ·
 * `MARKERS.md` §3.2·§4.2)는 본문이 자리마다 이름으로 부른다. 같은 두 문서 안에서도 **서술의
 * 곁근거로만 부르는 자리**는 안 든다(의존성 예산 §2.2 · 기동 §3.1). 정의역을 안 적으면 이
 * 열거가 인용 색인으로 자라거나 다시 좁아진다.
 *
 ...
 * 죽은 항이라 걷었고, 본문이 네 자리에서 축 설계의 근거로 드는 §2.3은 열거에 없었다.
 * **규정은 처음부터 지켜졌고 틀린 것은 열거의 완전성이다** — `CLI-INTERFACE.md` §7.1이
 * 2026-08-21에 자기 열거에 대해 같은 판정을 쓴 형태 그대로다.
 *
