...
describe("축 I — 안 붙는 자리와 안 재는 자리 (§9.6 「이 절이 재지 못하는 것」)", () => {
  ...
  test("[미규정] 잠금이 사용자에게 주는 자국이 «회색이 된 빈 버튼» 하나다", () => {
    // 결정 5가 잠금을 둔 목적이 *"«눌렀는데 아무 일도 안 난다»"*를 없애는 것인데, 그 자리의
    // 문면은 화면 몫이고 **오늘 비어 있다**(결정 2가 스스로 *"규격 미달"*로 적었다 · `K-329`).
    // 즉 왕복이 도는 동안 화면에 나는 변화가 레이블 없는 버튼의 잠김 하나다. 그것이 결정 5의
    // 목적을 채우는가를 §9.6도 §9.4도 안 든다 — 판정 필요.
    ...
    expect((button?.[1] ?? "").trim(), "이 축이 붉는 날은 `K-329`가 닫힌 날이다").toBe("");
  });
