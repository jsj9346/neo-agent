 ...
 *
 * **2026-08-27 — 모집단이 공집합을 벗어났다.** 그때까지 §9.4가 *"오늘 반입할 수 있는 화면이
 * 원격에 0장이다"*를 스스로 적었고 매니페스트의 `generated` 엔트리가 0건이라 실물 축이
 * **공집합에서 참**이었다. 화면(`/` · `index.html`)이 반입되면서 그 축이 **앵커 이름 전부를
 * 실제 원문에서 전수 대조한다** — 이 파일의 그린이 오늘 처음으로 «화면이 앵커를 든다»를 뜻한다.
 * 형제 `assets.contract.test.ts`의 `sha256` 축이 같은 반입으로 같은 전환을 했다.
 *
 * **그래도 이 파일은 주입 매니페스트를 남긴다.** 실물 하나가 그린인 것과 검사가 무엇을 재는지가
 * 다른 물음이기 때문이다 — 실물 축은 화면이 옳을 때 통과하는 것과 검사가 죽어 통과하는 것을
 * 스스로 구별하지 못한다. 아래 역검증이 앵커 하나를 뺀 화면에서 실패 목록이 실제로 채워지는
 * 것을 보이고, 그 대조가 실물 축과 **같은 순수 함수**를 지난다.
 *
 * ## 이 검사가 재지 못하는 것 — 셋
 *
 ...
 * 함수가 둘 다 받아야 하고, 그 순간 표기 둘이 **함께** 규격이 되어 원격 프롬프트의
 ...
 * 외부 도구가 뽑은 화면이다).
 *
 * 엔트리가 HTML 문서인가를 묻는 술어를 이 파일이 따로 들지 않는다 — `src/assets.ts`가 export하는
...
describe("앵커 집합 (WEB-UI §9.4 결정 7)", () => {
  test("집합이 비어 있지 않다 — 빈 목록의 조용한 그린을 통과로 읽지 않는다", () => {
    // 목록이 비면 아래 대조가 못 찾은 이름 0으로 전건 그린이 된다(`ARCHITECTURE.md` §2.6).
...
describe("전수 대조 — 실물 (WEB-UI §9.4 결정 8)", () => {
  test("모집단이 화면 하나다 — 아래 대조가 공집합에서 돌지 않는다", () => {
    // **이 자리는 표지였다.** 옛 단언은 «모집단이 공집합이다»였고, 자산이 반입되는 날 먼저
    // 붉으라고 놓인 것이었다. 2026-08-27에 실제로 붉었고 그때 고친 것은 이 단언이 아니라
    // 머리의 서술이다. 지우지 않고 방향을 뒤집어 남긴다 — 아래 실물 축이 위반 **목록형**이라
    // 모집단이 다시 비면 목록도 비어 조용히 그린이 된다(`ARCHITECTURE.md` §2.6). 반입물을
    // 걷어내거나 엔트리의 미디어 타입 한 글자가 어긋나는 변경이 정확히 그 형태로 통과한다.
    //
    // 하나인 근거는 §9.4 결정 1(*"화면은 하나다."*)과 결정 5가 그 키를 `/`로 못박은 것이고,
    // 상한(*"`generated` HTML 문서는 많아야 하나다"*)은 결정 12로 형제 파일이 따로 잰다.
    expect(documentEntries(ASSET_MANIFEST).map(([route]) => route)).toEqual(["/"]);
  });

  test("실물 대조에 위반이 없다", () => {
    // **오늘 이 단언이 처음으로 실물을 잰다.** 반입 화면의 원문에서 `ANCHOR_NAMES`의 이름
    // 전부를 `id="<이름>"`으로 찾는다. 재는 것은 **이름의 존재**뿐이고 그 자리가 옳은가는
    // 루트 `MILESTONE.md`의 C2가 진다 — 결정 8이 그 한계를 스스로 적었다.
    expect(REAL).toEqual([]);
    // 대조가 실제로 이름을 읽었다는 것을 함께 든다. 위 목록형 단언만으로는 이름 목록이
    // 비어도 그린이므로, 그 경우를 형제 축(`ANCHOR_NAMES.length`)과 여기서 함께 닫는다.
    expect(ANCHOR_NAMES.length, "대조할 이름이 없다 — 실물 축이 공허하다").toBeGreaterThan(0);
  });
});
