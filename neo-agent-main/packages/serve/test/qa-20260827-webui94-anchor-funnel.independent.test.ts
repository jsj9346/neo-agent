...
describe("축 4 — 조회 통로의 한계 (WEB-UI §9.4 결정 13)", () => {
  ...
  test("[미규정] 원천이 `undefined`를 돌려주면 통로가 그것을 조용히 흘린다", () => {
    // 결정 13이 던짐을 계약으로 든 근거는 침묵의 금지다. 오늘의 방어는 `null` 하나이고, 그
    // 좁힘을 받쳐 주는 것은 타입뿐이다 — 브라우저의 조회 API가 `null`만 돌려주므로 실경로는
    // 안 열린다. **적어 두는 이유는 그 방어가 타입에 기대고 있다는 사실 자체다.**
    expect(anchorElement(undefinedSource, "transcript")).toBeUndefined();
  });
...
describe("축 7 — 배선 스캐너의 사각 (WEB-UI §9.4 결정 13)", () => {
  test("[미규정] 통로가 조회 API를 부르는데 리터럴 스캐너는 0을 낸다", () => {
