 ...
 *
 * **§8.1이 그 위에 여섯을 더 세웠고 그중 넷이 이 파일에 산다.** ① 입력 알파벳이 연결 사건을
 * 든다(`StreamSignal`) ② 연결의 처분이 순수 함수의 답이다(`disposition`) ③ 생멸 사건이 수열
 * 상태를 초기값으로 되돌리고 종단은 그것을 흡수한다(`applySignal`) ⑤ **프레임의 내용은 순수
 * 층이 그것을 접수한 뒤에만 화면에 서고, 접수 여부는 순수 층의 답이다**(`frameVerdict` —
 * 2026-08-31 신설). 넷 다 형태가 하나다 — **판정의 정본을 배선과 나눠 갖지 않는다.** 배선이
 * 같은 답을 다시 지을 곳이 있으면 둘이 갈리고, 그 갈림이 V-1과 그 부수 발견이었다.
 *
 * **④·⑥은 이 파일에 없고, 없는 것이 판정이다.** ④(낡은 연결의 신호가 상태를 못 민다)와
 * ⑥(착지가 실패하면 배선이 멈춘다)은 배선의 성질이라 순수 층이 **원리적으로 못 잰다** —
 * ⑥은 화면을 봐야 알 수 있고, 접으면 그 통지가 다시 같은 화면 층을 거쳐 같은 던짐에 또
 * 걸린다(§8.1이 그 갈래를 기각한 근거). 그 둘의 첫 검증은 실브라우저다.
 *
...
/* -------------------------------------------------------------------------- *
 * 접수 — 판정이 착지보다 앞이다 (§8.1 계약 ⑤)
 * -------------------------------------------------------------------------- */

/**
 * 프레임 신호 하나에 순수 층이 답하는 것 셋(§8.1 「계약의 형태」).
 *
 * - `state` — 접은 뒤의 상태. **배선은 이 값을 쓰고 프레임에 대해 `applySignal`을 따로 부르지
 *   않는다.** 두 번 접으면 접수 답이 가리키는 상태와 배선이 든 상태가 갈릴 수 있고, 그 갈림이
 *   정확히 «판정의 정본이 둘»의 형태다.
 * - `disposition` — 그 상태에서 연결을 어떻게 할 것인가(계약 ②).
 * - `accepted` — 그 프레임의 내용이 화면에 서도 되는가(계약 ⑤).
 *
 * @typedef {{
...
/**
 * 프레임 하나를 접고, 그 접기의 답 셋을 함께 낸다.
 *
 * **왜 셋이 한 자리에서 나는가.** §8.1 계약 ⑤가 *"프레임의 내용은 순수 층이 그것을 접수한
 * 뒤에만 화면에 선다. 접수 여부는 순수 층의 답이고 배선이 다시 짓지 않는다."*로 못박았고,
 * 접수는 **접기 전 상태와 접은 뒤 상태를 함께** 봐야 정해진다. 술어를 따로 두면 배선이
 * «접기 전»을 인자로 넘겨야 하고, 그 인자를 잘못 넘기는 경로가 열린다 — 판정에 필요한 두
 * 값을 한 함수가 쥐면 그 경로가 표현 불가능해진다. (이름과 배치는 세부라고 정본이 적는다.)
 *
 * **접수의 정의는 상태에서 나온다 — 새 술어를 발명하지 않는다**(§8.1). 읽혔고 · 접기 전이
 * `live`였고 · 접은 뒤가 `broken`도 `gap`도 아니면 접수다. 정본 §8.1 접수표의 여섯 행이
 * 전부 이 세 항의 귀결이다:
 *
 * | 프레임 | 접수 | 이 정의에서의 도출 |
 * |---|---|---|
 * | 수열에 이어진 푸시 | ○ | `live` → `live` |
 * | 종료 고지 | ○ | `live` → `ended` — 종단이지만 `broken`·`gap`이 아니다 |
 * | 푸시가 아닌 프레임(`req`·`res`) | ○ | `step`이 상태를 안 바꾼다 |
 * | 못 읽은 줄 · 결함으로 판정된 프레임 | ✗ | `→ broken` |
 * | 갭을 낸 프레임 | ✗ | `→ gap` |
 * | 종단·갭인 채로 온 프레임 | ✗ | 접기 전이 `live`가 아니다 |
 *
 * **접수 ○가 «내용이 선다»와 같은 말은 아니다.** 셋째 행에서 서는 것은 계약 밖이라는 **고지**이고
 * 그것이 무엇이 되는가는 배선의 배치다(§9.6). 이 함수가 답하는 것은 «순수 층이 이 프레임을
 * 받아들였는가» 하나이며, 그 답이 거짓일 때 배선이 내용을 세우지 않는 것이 계약 ⑤다.
 *
 * **`read.ok` 항이 오늘 잉여인 것은 사실이나 안 걷는다.** `step`이 `live`에서 못 읽은 줄을
 * `broken`으로 옮기므로 셋째 항이 그것을 이미 덮는다. 그래도 세 항을 그대로 적는 이유는 이
 * 정의가 **`step`의 그 성질에 기대지 않게** 하기 위해서다 — 기대면 접수의 정본이 이 함수와
 * `step` 둘로 갈리고, 그것이 이 파일이 계약 ②·⑤에서 피하려는 형태 그대로다.
 *
 * **버리는 것이 아니다.** `accepted`가 거짓인 자리에 무엇이 서는가는 §8.1이 이미 정했다 —
 * 결함 문면 · 갭 문면 · 이미 선 종단 문면. `ARCHITECTURE.md` §2.6이 금지한 것은 자국 없이
 * 사라지는 것이고, 여기서는 내용 대신 사유가 선다. 그 사유의 원천이 이 답의 `state`다.
 *
 * @param {StreamState} state 접기 전 상태
 * @param {FrameResult} read 스트림이 실어 온 줄을 읽은 결과
 * @returns {FrameVerdict}
 */
export function frameVerdict(state, read) {
  const next = step(state, read);
  return {
    state: next,
    disposition: disposition(next),
    accepted: read.ok && state.phase === "live" && next.phase !== "broken" && next.phase !== "gap",
  };
}
