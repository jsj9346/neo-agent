 ...
 *
 * ## 부트가 지는 자리가 하나 늘었다 (2026-08-31 · §8.1 계약 ⑥ · §9.6 결정 13)
 *
 * **착지 실패를 잡는 것이 부트의 일이 됐다.** 계약 ⑥이 *"화면 층이 던지면 배선은 그 자리에서
 ...
 *
 * **곁따라 열거가 또 벌어진다.** 결정 12의 *"부트가 하는 것은 셋뿐이다"*가 세는 수가 하나 더
 * 오르고(테마 층에 이어 둘째다), **그 열거의 정정은 이 파일이 하지 않는다 — `K-348` 소관이다.**
 * §9.6 결정 13이 그렇게 넘겼다: *"그 열거의 정정은 `K-348`이 든다 — 이 항은 그 카드의 모집단이
 * 하나 늘었다는 사실만 적는다."* 계약의 실질(*"판정은 0이다"*)은 여기서도 지켜진다 — 무엇이
 * 던졌는지 값을 보고 갈래를 고르지 않으므로 고를 것이 없다.
 *
 * ## 이 파일이 매니페스트에 등재된 근거
 *
