 ...
 *
 * ## 착지 실패의 자국이 이 층에 산다 (결정 13 · §8.1 계약 ⑥)
 *
 * **[2026-08-31 신설]** 접기·뷰·그리기 중 하나가 던지면 배선이 멈추고 그 사실이 화면에
 * 서는데, 결정 13이 그 자국의 경로를 이 층의 **최소 경로 하나**로 못박았다 —
 * *"착지 실패의 자국은 그리기 층의 최소 경로 하나로 서고 접기와 뷰를 안 거친다."* 아래
 * `markStopped`가 그것이고, **위 분기 셋에 하나도 안 더한다**(그 함수 본문의 분기가 0이다).
 * 잡는 자리는 이 파일이 아니라 부트다(같은 항 — *"잡는 자리는 부트 하나다."*).
 *
 * ## 이 파일이 재지 못하는 것
 *
...
/**
 * 배선이 멈췄다는 **자국** 하나를 쓴다 — 정본은 §8.1 계약 ⑥과 §9.6 결정 13이다.
 *
 * 계약 ⑥이 *"화면 층이 던지면 배선은 그 자리에서 멈춘다"*와 함께
 * *"그 자국은 던진 층을 안 거친다."*를 들고, 결정 13이 그 자리를 이 층으로 못박는다 —
 * *"착지 실패의 자국은 그리기 층의 최소 경로 하나로 서고 접기와 뷰를 안 거친다."* 던진 것이
 * 그 둘 중 하나이므로 그 층으로 자국을 쓰면 같은 실패에 다시 걸린다. 그래서 이 함수는 상태도
 * 뷰 값도 안 받고 **앵커 표와 문면 하나**만 받는다.
 *
 * **자리는 `connection-status`다**(결정 13). 결정 6이 그 뜻을 «연결의 처분»에서 «세션이 지금
 * 무엇을 하고 있는가»로 넓혔고, *"배선이 멈췄다는 것은 그 물음의 답이다."* 새 앵커를 여는
 * 갈래는 화면 재생성을 요구하므로(§9 — 손질 금지) 같은 항이 이미 기각했다.
 *
 * **분기가 0이다.** 결정 13이 이 경로가 자기 자신은 못 던지는 근거를 결정 1의
 * *"그리기 층은 판정을 안 한다."*로 들고 *"분기가 0인 자리는 소진 검사도 0이다."*로 적는다.
 * 그래서 이 본문에 `if`·`switch`·`?`가 하나도 없다. **`paint`를 안 부르는 것도 같은 근거다** —
 * 부르면 뷰 값을 지어야 하고 그 순간 뷰 층을 거친다.
 *
 * **문면의 값은 이 층이 안 든다**(§12 — 값은 세부다). 지목은 부트가 하고 여기서는 받은
 * 문자열을 그대로 쓴다.
 *
 * @param {AnchorElements} elements
 * @param {string} text
 * @returns {void}
 */
export function markStopped(elements, text) {
