...
class Registry implements ApprovalRegistry {
  ...
  /**
   * id는 프로세스 안에서 단조 증가하는 수로 만든다. 도구 호출 id를 쓰지 않는 이유는
   * 그것이 모델이 만든 값이라 유일성을 우리가 보증하지 못하기 때문이고, 난수를 쓰지 않는
   * 이유는 §2.2의 허용 내장 넷에 `node:crypto`가 없기 때문이다. 이 값은 대기 레코드를
   * 가리키는 손잡이일 뿐이라 추측 불가능성이 요구되지 않는다 — §4가 루프백으로 닫아
   * 이 표면에 남이 없다.
   */
  #nextId = 1;
  ...
  ask(request: ApprovalAsk, signal: AbortSignal): Promise<ApprovalAnswer> {
    ...
    const id = `approval-${String(this.#nextId)}`;
    this.#nextId += 1;
