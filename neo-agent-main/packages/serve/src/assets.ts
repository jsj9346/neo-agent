...
export const ASSET_MANIFEST = {
  ...
  "/client/anchors.js": {
    ...
    // 정의에 들지 않는다"*. 이 파일은 브라우저가 URL로 받아 가는 종류이므로 배선이 0건인
    // 오늘도 자산이다. 같은 항이 이 주석을 그 명시의 근거로 이름 붙였다(구현 주석·독립
    // QA·보드가 같은 오독에 각각 도달했다).
    //
  ...
  "/client/wiring.js": {
    ...
    //
    // **위 `/client/anchors.js`와 같은 상태다** — 오늘 이 모듈을 브라우저가 받아 갈 배선이
    // 0건이고, 그럼에도 등재가 서는 근거는 §9.2가 판별을 종류로 못박은 것이다(2026-08-27).
    // 형제와 같은 갈래를 고르는 것이 판단이고, 갈래를 달리 고르면 같은 디렉터리 안에서
    // 처분이 갈려 그 물음이 파일마다 따로 열린다.
    origin: "authored",
    file: "wiring.js",
    contentType: "text/javascript; charset=utf-8",
  },
