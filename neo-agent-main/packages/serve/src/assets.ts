...
export const ASSET_MANIFEST = {
  ...
  "/client/main.js": {
    ...
    // 화면이 여는 `<script type="module">`이 이 키다 — 형제들은 이 모듈의 임포트 그래프로
    // 딸려 온다. 근거는 층이다: 진입점이 둘이면 로드 순서가 화면의 성질이 되고
    ...
    // 같은 값을 들도록 함께 고친다.
    //
    // **형제들과 같은 상태이나 근거가 한 겹 낫다.** §9.2의 자산 정의(«브라우저가 URL로 받아
    // 가는 것»)를 이 파일은 직접 만족한다 — 화면이 여는 유일한 `<script>`가 이것이다.
    //
    // **[정정 — 2026-08-27] 이 자리는 «그 화면이 오늘 0장이므로 등재가 내다봄 위에 선 것은
    // 마찬가지다»라 적고 있었고, 같은 반입이 위 `/` 엔트리를 세우면서 그 서술이 낡았다.**
    // 그때 참이었던 관측(화면 0장)은 2026-08-26 §9.4 실독 시점의 것이다. 오늘 화면은 실재하고
    // 그 원문이 이 키를 여는 `<script type="module">` 하나를 든다 — 즉 이 엔트리는 형제들과
    // 달리 **내다봄이 아니라 실물 위에 선다**(그 «하나»를 `assets.contract.test.ts`의 §9.5
    // 결정 4 축이 화면 원문에서 잰다). 그래도 **`K-311`이 연 물음을 이 등재가 닫지 않는다** —
    // 그 카드가 묻는 것은 받아 갈 배선이고, 배선은 여전히 0건이다.
    origin: "authored",
    file: "main.js",
    contentType: "text/javascript; charset=utf-8",
  },
  ...
  // 든다»를 만족하는 표준 미디어 타입이다. 값의 형식(대소문자·공백)을 정본이 안 정해 술어
  // 쪽에 처분이 쌓이는 것은 `K-314`가 진다 — 이 자리에서 정하지 않는다.
  "/tokens.css": {
    origin: "generated",
    file: "tokens.css",
