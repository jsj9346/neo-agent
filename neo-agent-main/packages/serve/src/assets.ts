 ...
 * §9.1의 집합 동일성 중 루트 쪽 축에서 위반으로 읽히므로, **그 축을 재는 계약 테스트가 이 파일 이름을
 * 명시적으로 제외한다** — 제외를 코드가 들지 않으면 다음이 그것을 자산으로 오해하거나
 * 검사를 느슨하게 고쳐 통과시킨다.
 *
 * ## 불변 캐시를 두지 않는다 (§9.1)
 *
 ...
 * 타입 레벨에서 닫힌다."
 *
 * **오늘 엔트리는 `authored` 둘이다.** 반입 자산이 0건이라 `generated`가 비어 있고(§12 —
 * 화면 자산은 아직 뽑지 않았다), 여기 있는 둘은 §9.3이 이 레포의 소유로 판정한 프로토콜
 * 행동 모듈이다. **이 상태를 반입 자산이 검증된 것으로 읽지 않는다** — `sha256` 대조 축은 오늘
