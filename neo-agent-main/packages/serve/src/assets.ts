 ...
 * 모듈과, §9.4 결정 7이 이 레포의 정본으로 판정한 앵커 상수, 결정 13이 이 레포의 소유로
 * 판정한 배선 층의 순수 부분, 그리고 §9.5 결정 4가 화면에 **하나만** 열게 한 스크립트
 * 진입점이다. **이 상태를 반입 자산이 검증된 것으로 읽지 않는다** — `sha256` 대조 축은 오늘
 * 공집합에서 참이고, 그 사실을 계약 테스트의 머리가 든다.
 *
