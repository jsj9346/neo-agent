 ...
 *
 * 예외가 본문에 한 건 남았다 — `store` 본업 방어는 이관 짝이 없다. **그 자리를 드는 것은
 * 미규정 표시가 아니라 날짜를 박은 판정 주석이다**(2026-08-28 정정 — 그때까지 이 문장이
 * 미규정 표시라 적고 있었고 본문의 그 표시는 0건이었다. 아래 머리 ↔ 본문 대조가 방향 하나만
 * 재므로 틀린 쪽으로 넘어가지는 않았으나, 부류를 틀리게 들면 다음 사이클이 없는 표시를 찾는다).
 ...
 * 단위 분해(`documentUnits`)와 대조 판정은 그대로 여기 산다. **그래서 아래 한계 넷의
 * 소유자가 갈린다** — 2와 3은 임포트한 파서의 성질이고, 1과 4는 이 파일의 것이다.
 * 목록을 줄이지 않는 이유는 **이 파일이 무엇을 증명하지 않는지가 소유자와 무관하게 같기
 * 때문**이다. 파서 쪽 한계가 고쳐지면 그때 이 목록에서 빠진다.
 *
 * 옮기면서 **좁아진 자리가 넷**이고, **오늘 그 넷이 전부다** — §3.4가 정하지 않아 임의로
 * 고른 자리는 하나도 남지 않았다. **머리가 실물보다 넓게 주장하지 않도록** 넷을 전부 여기
 * 적는다 — 이 목록은 이 파일이 무엇을 증명하지 **않는지**의 정본이다.
 *
 * **임의로 고른 자리가 셋에서 하나로, 다시 영으로 준 것은 계약이 그 셋을 차례로 닫았기
 * 때문이다.** 제목 줄·빈 줄의 처분은 **N-2**가, 같은 문면을 담은 단위가 여럿일 때의 처분은
 * **N-3**이 정했고(2026-08-14), 마지막 하나였던 표 칸 경계의 코드 스팬 파이프는 §3.4가
 * 2026-08-17에 맨 구분자 쪽으로 **추인**해 닫았다(`DOC-CITATION.md:306` · `K-101` — 아래
 * `pushTableCells`의 주석이 그 근거를 든다). 셋 다 이제 임의 선택이 아니라 계약 준수이므로
 * 이 목록에서 빠진다.
 *
 ...
 * 던진다. **본문의 미규정 표시는 §3.4 축에서도 그 밖에서도 오늘 영이다** — 위 이관 절이 든
 * `store` 항은 판정 주석이 드는 자리라 표시가 아니고, §3.4가 아니라 플랜 §9.5가 안 정한
 * 자리라 아래 넷과 부류도 다르다. 새 미규정 선택을 본문에 넣고 머리에 안 적으면 두 번째가
 * red다. **모집단이 0이라 그 대조는 오늘 공허하게 참이고**, 형제가 그것을 알고 합성 입력으로
 * 추출기가 도는 것을 따로 고정한다 — 0이 본문에 없어서인지 추출기가 죽어서인지를 가르는 자리다.
 *
 * ### 옮기면서 좁아진 자리 — 넷
 *
 ...
 *    이 파일이 닫지 않는다.
 *
 * 이 절은 인용부호를 쓰지 않는다. **2026-08-17 정정 — 그 근거로 적혀 있던 것이 반증됐다.**
 * 그때까지 이 자리는 `DOC-CITATION.md` §1이 규약의 범위를 `neo-agent-main/docs/*.md` 안의
 * 주소 표기로 그었고 코드 대상 인용은 §6 U-b로 열어 두었으므로 `.ts` 주석은 원리적으로 대조
 * 축 밖이라고 적고 있었다. §6 U-b의 2026-08-17 판정이 대조 축(U-1 · D-1 · D-2)이 걸리는
 * 자리를 각 패키지의 테스트 디렉터리까지 넓혔으므로 **이 파일의 주석도 축 안이고**, 강제는
 * 게이트가 아니라 이 머리가 든다. 위쪽 문단들이 쓰는 인용부호도 그래서 축 안이다 — 이 머리에
 * 실재하는 갈래는 겹화살괄호 넷이고 별표 형식과 평문 큰따옴표는 0건이다(2026-08-28 실측).
 * 그 넷의 처분은 이 사이클이 하지 않았다.
 *
 * **2026-08-28 정정** — 그때까지 이 자리는 남은 갈래를 별표 형식과 평문 큰따옴표로 들고 있었고,
 * 그 둘은 이 머리에 한 건도 없었다. 이 문장이 무엇이 미처분으로 남았는가를 다음 사이클에
 * 넘기는 자리라, 부류가 틀리면 처분해야 할 갈래를 다른 갈래로 보낸다.
 * 인용부호를 안 쓰는 이유 자체는 그대로 남는다 — **이 자리의 인용을 대조하는 기계가 없다.**
 * `cited` 계열이 재는 것은 `docs/*.md`의 문면이지 자기 주석이 아니다. 대조받지 않는
 * 인용부호는 원문이 이렇다는 신호만 주고 그 신호가 참인지 아무도 묻지 않는다.
 */
 ...
 * 전처리에 필요한 것이 이만큼이고, 형제 파일(`package-boundary.contract.test.ts`)이 같은
 * 이름의 파일-로컬 판본을 이미 든다. **그쪽 판본은 문자열 리터럴까지 안다** — 이 자리에
 * 그것이 필요 없는 것은 아래 한계 문단이 든 이유 때문이다. 지목에 줄번호를 안 쓴 것은 그
 * 형제가 오늘 그 자리에서 옮겨 갔기 때문이다(2026-08-22).
 *
 ...
 *
 * 아래 두 파서는 §3.4가 정한 것을 그대로 옮긴다. S-6이 어디가 인용부호인가를,
 * S-5가 단위가 무엇인가를, S-4가 대조 코퍼스에서 무엇을 빼는가를 정한다.
 ...
 *    N-1이 목록 항목 안의 코드 펜스는 항목이 단위이고 하위 항목은 바깥 항목이 단위라고 정한다.
 * 2. 그런 줄이 뒤따르는 빈 줄.
 ...
 *
 * **맨 구분자가 칸을 가르고, 코드 표기 안인지 묻지 않는다** — §3.4가 2026-08-17에 추인으로
 * 닫았다(`DOC-CITATION.md:306` · `K-101`). 그날까지 이 축은 정본에 문면이 없어 여기가 임의로
 * 고른 값이었고, 추인의 근거가 «구현이 이미 그 값이기 때문»이므로 **이 함수의 계산이 그대로
 * 정본의 값이 됐다.** 유일한 예외는 직전 문자가 백슬래시인 이스케이프(`\|`)다.
 */
...
describe("DOC-CITATION §3.4 S-5 — 렌더링 단위 분해", () => {
  ...
  it("N-1 실물 — 항목 안 펜스가 항목과 한 단위다", () => {
    ...
    // 고정하는 이유는 실물의 연속 형태가 마커 줄 → 빈 줄 → 들여쓴 펜스라 합성 표본과
    // 다르기 때문이다.
...
describe("DOC-CITATION §3.4 S-6 — 인용부호 구간 추출", () => {
  ...
  it("역검증 — 곧은 따옴표만 보는 옛 추출은 곡선 인용을 놓친다", () => {
    // 통과만 확인하는 검사로 퇴화하지 않도록 구멍이 실재했음과 그것이 닫혔음을 한 자리에 둔다.
...
describe("PROVIDERS §2.2 `AnthropicClientConfig` 블록 ↔ `client.ts`", () => {
  ...
  it("검사기가 필드 누락을 실제로 잡는다 (역검증)", () => {
    // D-1이 정확히 문서 블록에 필드 하나가 없다는 형태였다. 그 형태를 표본으로 재현한다.
...
describe("DOC-CITATION U-1 — 이 사이클이 들여온 인용의 문면 일치", () => {
   ...
   * 경유한다는 종결형으로 의역하고 있었다. 처분은 **인용부호를 벗겨 서술로 바꾸는 것**이었고
   * (§2.1의 원문은 그 자리에서 문장을 끝내지 않고 그 밖의 I/O로 이어지므로 종결형 부분
   * 문자열이 존재하지 않는다),
   * 그래서 그 항목은 목록에서 빠졌다 — 인용이 사라졌으므로 대조할 대상이 없다.
   *
   * **이 절이 재지 못하는 것은 파일 머리의 한계 넷이 든다** — 손으로 유지되는 목록은
   * 그중 넷째다. 여기에 다시 적지 않는다: 두 자리에 같은 목록을 두면 어느 쪽이 최신인지 알
   * 수 없어져 둘 다 신뢰를 잃는다.
   */
  /** 판정 결과. 닫힌 유니온이라 어느 갈래로 깨졌는지가 타입에서 사라지지 않는다 */
  type CitedVerdict =
  ...
  /**
   * 실패 메시지는 두 갈래다 — 문면이 없는 것과 원문이 사라진 것은 고칠 곳이 다르다.
  ...
  // 자기 자신을 1건도 고르지 못한다(플랜 §9 E-4가 요구한 검증 조건과 실행 명령의 일치가 깨진다).
  ...
  it("머리 주석이 판정 방식의 한계 넷을 전부 든다", () => {
    // `K-069`가 지적한 결함은 머리가 실제보다 넓게 주장하는 것이다. 판정 방식이 바뀌면
    // 주석이 낡고, 낡은 주석은 다음 감사가 이 파일을 실물보다 강하게 읽게 만든다.
    //
    // **이름의 수가 일곱 → 다섯 → 넷으로 준 것이 그동안의 처분이다.** §3.4가 N-2·N-3으로
    // 임의 선택 둘을(2026-08-14), 표 칸 경계의 맨 구분자 추인으로 마지막 하나를(2026-08-17 ·
    // `K-101`) 계약으로 흡수했으므로 셋 다 임의로 고른 자리가 아니다. 이름의 수가 실물과
    ...
    // 흡수된 셋이 «임의로 고른 자리»의 절로 다시 실리지 않는다 — 계약이 정한 것은 한계가
    // 아니다. 표제를 두 형태 다 막는다: 되살리는 편집은 이 둘 중 하나를 쓴다.
    for (const stale of ["임의로 고른 자리 — 셋", "임의로 고른 자리 — 하나"]) {
      expect(header, `계약이 흡수한 자리가 «${stale}»로 되살아났다`).not.toContain(stale);
    }
