 ...
 * 아니라 지금 활성인 마일스톤이 M5인지를 재게 됐고 red가 됐다.
 *
...
// ---------------------------------------------------------------------------
// 축 A2 — §5 「그 문서는 번호 절을 든다」
//
// 2026-09-03까지 이 축은 축 I의 `[미규정]` 관측 고정이었다(번호 절 0개). §5가 요구한
// 적이 없어 판정하지 않았고, 그 미규정을 `K-444` 설계가 §5 명문화로 닫았다.
//
// **기대값의 출처는 §5의 열거 하나이고 실물에서 읽지 않았다.** 그 열거는
// `§1 경계 · §2 …` 꼴의 **번호 + 짧은 이름**이므로 이 축이 재는 것도 거기까지다 —
// 절 제목의 전문이 아니라 접두다. 실물 §1의 부제 「— 이 문서가 드는 것과 안 드는 것」은
// `PUBLIC-TREE.md` 어디에도 없어, 그것을 기대값에 적으면 그 순간 정본이 아니라 실물에서
// 읽은 것이 된다(2026-09-03 검증 F-5 — 그전까지 이 축이 그렇게 적혀 있었다).
// ---------------------------------------------------------------------------

describe("PUBLIC-TREE.md §5 — COMPLIANCE.md가 번호 절을 든다", () => {
  /**
   * §5의 열거를 그대로 옮긴 접두 넷. 순서까지가 계약이다.
   * 부제가 붙는 절이 있어 전문이 아니라 접두로 잰다(위 머리 주석).
   */
  ...
  it("§5가 그 요구를 문면으로 든다 — 기대값의 출처", () => {
    expect(PUBLIC_TREE).toContain("**그 문서는 번호 절을 든다.**");
    expect(PUBLIC_TREE).toContain(
      "§1 경계 · §2 절대 이식 금지 · §3 이식 시 반드시 지킬 원칙 · §4 근거 분류 체계",
    );
  });

  it("최상위 번호 절 넷이 §5가 든 순서 그대로 있다", () => {
    // §5: *"요구는 최상위 번호 절까지이고 소절은 아니다"*. 술어를 `##` 하나로 좁힌다 —
    // `#{2,6}`으로 세면 §5가 **허용하는** 소절 추가가 이 항을 붉힌다(2026-09-03 F-3의
    // 역검증에서 실제로 났다). 정본이 정한 것보다 더 많이 재는 검사는 계약이 아니라
    // 계약의 참칭이다.
    //
    // 접두 일치가 함께 재는 것이 §5의 *"부류 제목의 문자열은 번호가 붙어도 안 바뀐다"*다.
    // 그 제목을 이름으로 부르는 자리는 이 트리 **밖**에 여섯 있으나(`.claude/`의 에이전트
    // 정의 다섯 + 루트 규약), `ARCHITECTURE.md` §2.22(제품 트리는 자족한다)가 제품
    // 테스트에서 그 트리를 읽는 것을 막는다 — 여기서 재는 것은 **트리 안의 접두 보존**
    // 까지이고, 트리 밖 여섯을 실제로 재려면 하네스 쪽 검사가 필요하다.
    const numbered = COMPLIANCE.split("\n").filter((l) => /^## \d+(\.\d+)*\.? /.test(l));
    expect(numbered).toHaveLength(EXPECTED_PREFIXES.length);
    expect(EXPECTED_PREFIXES.map((prefix, i) => numbered[i]?.slice(0, prefix.length))).toEqual(
      EXPECTED_PREFIXES,
    );
  });
});
