...
// 적이 없어 판정하지 않았고, 그 미규정을 `K-444` 설계가 §5 명문화로 닫았다. 기대값은
// 그 절이 든 절 구분 그대로이고 실물에서 읽지 않았다.
// ---------------------------------------------------------------------------
...
describe("PUBLIC-TREE.md §5 — COMPLIANCE.md가 번호 절을 든다", () => {
  ...
  it("절 번호가 부류 제목의 문자열을 안 깨뜨린다 — 번호는 접두다", () => {
    // §5: *"부류 제목의 문자열은 번호가 붙어도 안 바뀐다"*. 이 트리 밖에서 그 제목을
    // 이름으로 부르는 자리가 있고, 접두 규칙이 그 자리들을 살린다.
    expect(COMPLIANCE).toContain("## 2. 절대 이식 금지");
    expect(COMPLIANCE).toContain("## 3. 이식 시 반드시 지킬 원칙");
  });
});
