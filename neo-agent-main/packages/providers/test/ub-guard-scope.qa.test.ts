 ...
 * 재판정 절이 든다. 여기 남는 축은 다섯이고 **본문 배너의 번호는 0에서 시작한다** —
 * 아래 열거는 그 번호를 그대로 쓴다.
 *
 * 0. **모집단이 자리 그 자체인가** — 열거가 손 목록으로 좁아져 있지 않다는 것을 재고, 비면
 *    통과가 아니라 실패로 낸다. 아래 축들이 전부 이 열거를 모수로 쓰므로 여기가 먼저 선다.
 ...
 *    소유는 `K-150`.
 * 4. **파일 축 대조의 덮개와 머리 절단의 경계, 그리고 강제 선언** — 인용부호 셋 전부에 대한
 *    파일 축 대조와, 한 블록 주석 안에서 소속이 갈리는 배치 둘의 실단언. 그 둘은 2026-08-18까지
 *    등급 없이 오늘 값만 고정했고, 같은 절의 후속 판정이 소속을 렉싱으로 못박아 승격됐다.
 *    **강제 선언 축이 이 배너 안에 산다** — 자리 안 파일의 머리가 이 계약의 선언을 드는가이고,
 *    술어는 §6 U-b 2026-08-19 후속 판정 문면에서 도출했다(머리 덩어리 안 문자열 둘의 동시
 *    출현). 위 심기 배치 둘도 그 술어가 찾은 자리에 심는다 — 심을 자리를 고정 문장으로 잡으면
 *    모집단이 그 문장을 든 다섯 파일로 조용히 좁아진다.
 *
 * **2026-08-28 정정** — 그때까지 이 열거는 번호를 1에서 시작해 본문 배너와 한 칸씩 어긋났고,
 * 배너를 안 가진 강제 선언 축을 다섯째 항으로 들면서 실재하는 배너 0을 통째로 빼고 있었다.
 *
...
describe("DOC-CITATION §6 U-b — 대조 축의 단위는 파일이다", () => {
  ...
  //       최소값은 오늘만 맞는 숫자이고, 그 위로 한 칸 올려 잡는다.
  //   (c) 더 올리지 않는 이유는 상한의 본래 일(진짜 멈춘 것을 잡는 것)의 비용을 이 축 하나가
  //       아니라 전량 런 전체가 지기 때문이다.
  //
  // 지원 경계 — 이 값은 전량 런 1본 조건 아래에서만 뜻이 있다(§2.21). 동시 2본 이상에서 나는
  // red를 그 절은 계약 위반으로 세지 않는다. 소급도 하지 않는다: 형제 축(강제 선언 축)과 전역
  // `testTimeout`·공용 대기 헬퍼는 이 처분이 안 건드린다.
