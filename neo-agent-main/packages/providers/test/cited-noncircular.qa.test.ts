 ...
 * S-4 · S-5 · S-6 · S-7 · N-1~N-4 · Q-1~Q-3을 옮긴 것이 맞는가.
 *
 ...
 *   않는다. 이 파일의 범위는 §3.4 S-4 · S-5 · S-6 · N-1~N-4 · Q-1~Q-3과 대상 파일 머리
 *   주석의 주장뿐이다.
 ...
 *   바뀌었다** — N-1~N-4 · Q-1~Q-3이 그 일곱 자리를 전부 정했다. 오늘 남은 [미규정]은 **둘**
 *   이다: 표 칸 경계의 코드 스팬 `|`(대상 파일 자신이 머리에 «임의로 고른 자리»로 든 것) ·
 *   여는 펜스와 닫는 펜스의 들여쓰기가 어긋난 쌍(2026-08-15에 들였다 — Q-2가 폭을 풀면서
 *   생긴 자리다). 뒤엣것은 오늘의 읽기조차 고정하지 않고 `it.todo`로만 세워 둔다.
 *
...
/* ------------------------------------------------------------------------ *
 * S-6 · Q-1~Q-3 — 무엇이 코드이고 무엇이 인용부호인가
 * ------------------------------------------------------------------------ */

/** 한 줄 안에서 닫히는 이중 백틱 코드 스팬. CommonMark의 여는/닫는 런 규칙을 이 폭만 쓴다 */
const DOUBLE_BACKTICK_SPAN = /``[^\n]*?``/g;

describe("DOC-CITATION §3.4 S-6 · Q-1~Q-3 — 코드 표기와 인용부호의 부류", () => {
  ...
  it("Q-2 — 마커 «없는» 들여쓰기 블록은 코드 표기가 아니므로 그 안의 큰따옴표는 인용부호다", () => {
    ...
    // Q-1로 보낸다 — *"«들여쓰기 코드 블록»은 마커가 없는 것을 말한다 … 들여쓴 **펜스**는
    ...
    // 연속이다"*(2026-08-14). **옛 검사는 이것을 «4칸 이상 들여쓴 펜스가 0건»으로 근사했는데
  ...
  // **[미규정] — 여는 펜스와 닫는 펜스의 들여쓰기가 어긋난 쌍.** Q-1은 인라인 스팬에 대해
  // **런 길이 일치**를 술어의 일부로 못박고 그 이유(유령 구간 → 근거 없는 red)까지 들었는데,
  // 펜스에 대해서는 여는 줄과 닫는 줄의 **들여쓰기**가 어떤 관계여야 하는지를 한 글자도 말하지
  // 않는다. Q-2의 2026-08-15 명문화도 «몇 칸을 들여썼든»으로 폭만 풀 뿐 쌍을 말하지 않는다.
  // **방향이 침묵 쪽이라 중요하다** — 어긋난 닫는 줄이 펜스를 안 닫으면 마스킹이 문서 뒤를
  // 삼켜 인용부호 구간이 조용히 사라진다(같은 파일 «근거 없는 그린 후보» 검사가 든 형태).
  // 오늘의 구현은 들여쓰기를 안 보고 닫아 그 침묵이 안 나지만, 그것은 계약이 정한 것이 아니라
  // 구현의 선택이다. 여기서 임의로 고정하지 않는다 — 판정은 리포트가 올린다.
  it.todo("[미규정] 여는 펜스와 닫는 펜스의 들여쓰기가 어긋난 쌍 — §3.4가 정하지 않는다");
...
describe("DOC-CITATION §3.4 S-5 · N-1 · N-2 — 단위 분해", () => {
  ...
  it("[미규정] 표 칸을 가를 때 코드 스팬 안의 `|` — §3.4가 정하지 않는다", () => {
    // 대상 파일이 머리에서 «임의로 고른 자리»로 든 하나다. §3.4는 표 칸 경계를 정하는 문면이
    // 없다 — S-5는 «표 한 칸»을 형태로만 들고, N-1은 다섯 형태끼리의 겹침만 처분하며, Q-1은
    // 코드 표기 안의 큰따옴표만 말한다. 오늘의 읽기(이스케이프한 `\|`만 내용)를 고정만 한다.
