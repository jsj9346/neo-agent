 ...
 * S-4 · S-5 · S-6 · N-1~N-4 · Q-1~Q-3 · Q-6을 옮긴 것이 맞는가.
 *
 * **2026-08-28 정정** — 그때까지 이 줄이 S-7을 그 목록에 함께 들고 있었고, 아래 범위 문장은
 * 안 들고 있었다. **머리 두 줄이 서로를 반증했고 실물은 뒤쪽이 옳다** — S-7 전용 축은 없고
 * 그 규칙은 N-4 축의 근거로만 나온다.
 *
 ...
 * 2026-08-16에 들어왔고, 그날까지 이 파일이 미규정 마커로 세워 둔 자리(여닫 펜스의 들여쓰기)를
 * 정본이 답한 것이다.
 *
 ...
 *   전용으로 좁혔다. **다만 그 모듈의 파서까지 대상 밖인 것은 아니다** — 인용부호 구간
 *   추출(`quoteSpans`)은 대상이 임포트하는 그 모듈에서 그대로 와서 아래 축들에 걸리고
 *   (§6 U-e 2026-08-15 판정이 정본을 그리로 옮겼다), 대상 자신에서 오는 것은 단위 분해와
 *   대조 판정이다. 아래 적재기가 그 둘을 갈라 두었고 존재 확인도 대상 쪽 이름으로 한다.
 *   **2026-08-28 정정** — 그때까지 이 항이 재는 것을 대상 파일의 파서로만 들고 있었다.
 * - 회색지대는 판정하지 않는다. 미규정 마커가 붙은 `it`은 그날의 읽기를 고정만 하며
...
describe("DOC-CITATION §3.4 S-6 · Q-1~Q-3 · Q-6 — 코드 표기와 인용부호의 부류", () => {
  ...
  it("Q-6 — 여닫 펜스의 들여쓰기가 어긋나도 닫힌다 (자리는 닫기 판정에 안 든다)", () => {
    // **이 자리는 2026-08-15까지 미규정 마커였다.** 그때 §3.4는 여는 줄과 닫는 줄의 들여쓰기가
...
describe("머리 주석의 주장 ↔ 실물 — 대상 파일과 이 파일", () => {
  ...
  it("머리가 한계 목록을 정본이라 부르면 본문의 미규정 마커 판단도 거기 있어야 한다", () => {
    // 머리는 한계 목록이 **이 파일이 무엇을 증명하지 않는지**의 정본이라 적는다. 본문이 그
    // 목록 밖에서 판정을 바꾸는 미규정 마커 선택을 들면 머리가 실물보다 강하게 읽힌다.
    //
    ...
    expect(
      unlisted,
      `머리의 한계 목록이 본문의 미규정 마커 판단 ${unlisted.length}건을 들지 않는다:\n  ${unlisted.join("\n  ")}`,
    ).toEqual([]);
