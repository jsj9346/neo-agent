 ...
 *   - `docs/COMPACTION.md` §10 — **미결**: "`contextWindowTokens`의 미지 모델 보수 기본값 —
 *                               수치는 구현 시 확정(경고 표시가 계약)"
 *
 * §10이 수치를 명시적으로 미결로 남겼으므로 이 파일은 **수치를 단언하지 않는다.**
 * 검증 대상은 구조(`{ tokens, known }`)와 §3이 소비하려면 반드시 참이어야 하는 성질
 * (양수·유한·결정적)이다.
 *
...
describe("기지 모델 (COMPACTION §8)", () => {
  ...
  test("[미규정 A-8] 어떤 모델이 테이블에 있어야 하는지는 계약이 정하지 않는다", () => {
    ...
    // 구조가 성립하는지**만 단언하고 membership은 판정 요청으로 남긴다.
