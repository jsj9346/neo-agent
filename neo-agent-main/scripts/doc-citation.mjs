 ...
 *
 * **[미규정] 여는 줄과 닫는 줄의 «컨테이너»가 어긋난 쌍**(여는 줄은 인용 블록 안, 닫는 줄은
 * 밖). §3.4는 인라인 스팬에 대해서만 여닫의 관계를 정하고(런 길이) 펜스에 대해서는 들여쓰기도
 * 컨테이너도 말하지 않는다. 여기서 임의로 정하지 않고 **어긋나도 닫는** 오늘 동작을 유지한다 —
 * 안 닫으면 마스크가 문서 뒤로 번져 침묵이 되기 때문이다. 판정은 `K-118`이 든다
 * (같은 계열의 들여쓰기 축은 `cited-noncircular.qa.test.ts`의 `it.todo`가 이미 들고 있다).
 *
