 ...
 *
 * **이 대응(갈래 이름 = 출력 문자열)은 관행에서 온 [추정]이다** — `boundary-index.mjs`의
 * `VIOLATIONS`·`DOC-STATUS.md` §5.1 말미가 같은 형태를 쓰지만 `LORE.md` §4.2 자신은 그것을
 * 문면으로 못박지 않는다. 이 집합이 닫혀 있다는 것(=§4.2 표와 1:1)은 계약이다.
 */
 ...
 *
 * §4.2는 §4의 범위를 *"`## 4.`와 그 다음에 처음 나오는 `###` 사이"*로 정한다. 여기서는 다음
 * `##`도 범위를 닫는다 — §4가 소절을 잃는 날 범위가 §5까지 흘러 들어가 남의 표를 이름 사전
 * 행으로 읽는 것을 막는 fail-closed다. 오늘 `LORE.md`는 `### 4.1`이 `## 5.`보다 앞이라 두
 * 읽기가 같은 값을 낸다. 이 보강은 §4.2 문면이 정하지 않은 것이다. [미규정]
 *
 * @param {string} source
