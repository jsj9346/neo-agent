...
// §3을 지킨 머리만 갖추면 게이트가 그린이었다. 아래 대조가 그 자리를 닫는다.
//
// 집합 대조가 실행부에 있는 것은 §5.1의 계약이다 — 집합 대 집합이라 순수 판정이 만들 수 없는
// 값이고, 그래서 §3.1의 `Violation`을 늘리지 않는다. 나머지 재료(«행 파싱 실패»·«구간이 안
// 닫혔다»)는 순수 파서가 만들고 여기는 라벨과 종료만 맡는다.
// ---------------------------------------------------------------------------
 ...
 * 찍고 있어 §5.1의 저 문장이 갈래 하나에서 거짓이었다. 문서를 낮추지 않고 코드를 맞춘다.
 *
 * **여기 키로 서는 것만 §5.1의 갈래다.** 나머지 사유는 표 자체에 닿지 못한 경우라 fail-closed
 * 그물이고(§5.2 말미), 둘을 같은 라벨로 부르면 *"행 하나가 틀렸다"*와 *"표를 못 읽었다"*가
 * 같은 주소를 갖게 된다. 목록으로 두는 것이 조건식보다 나은 이유는 §3.6이 2026-08-23에
 * 실물로 보여줬다 — 조건식은 새 사유가 붙어도 안 붉고 그 사유를 조용히 «fail-closed»로 떨어뜨린다.
 */
const TABLE_FAILURE_LABELS = {
  "row-malformed": "행 파싱 실패",
  // §3.6 C-5 — 파일 끝까지 안 닫힌 HTML 주석·펜스. 순수 파서가 재료를 만들고(C-7이 마스킹을
  // 거기 두었다) 여기가 이름을 붙인다.
  "inert-unclosed": "구간이 안 닫혔다",
};

const table = parseStatusTable(readFileSync(tablePath, "utf8"));
if (!table.ok) {
  const label = TABLE_FAILURE_LABELS[table.reason] ?? "fail-closed";
  console.error(
