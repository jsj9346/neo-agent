 ...
 * 시작이 아니었다는 뜻이다(아포스트로피 한 개가 대표적인 경우다).
 *
 ...
 * `/["']/`처럼 안에 `/`나 따옴표를 품은 형태가 전부 한 구간으로 잡힌다. 정규식
 * 리터럴은 **줄을 넘지 못한다**는 문법을 그대로 쓰므로, 개행을 만나면 그 `/`는
 * 정규식의 시작이 아니었다는 뜻이다.
 *
 * **닫는 후보가 `//`·`/*`를 여는 자리면 닫지 않는다 — `-1`이다.** 이것이 없으면
 * 나눗셈 `/`가 연 유령 구간이 뒤따르는 실물 주석의 **첫 `/`에서 닫히고**, 커서가
 * 그 주석 토큰 한가운데(`//`의 둘째 `/`)에 선다. 그 자리에서 `//*`의 `/*`가 블록
 * 주석으로 읽혀 짝 없으면 **EOF까지 통째로 지웠다**(2026-08-23 `K-244` V-1 —
 * `const r = total / count; //* 메모` 한 줄로 그 아래 임포트 전부가 사라져 이 게이트가
 * 코어 예산 위반을 조용히 통과시켰다). `-1`을 돌리면 호출부가 「그 줄의 나머지를
 * 원문 그대로」로 넘어가므로 방향이 다시 **덜 지우는 쪽** 하나가 된다.
 *
 * 진짜 정규식은 이 자리에 거의 걸리지 않는다 — 닫는 `/` 뒤에 오는 것은 플래그(글자)나
 * 연산자·구분자이고, 안에 `//`를 품은 `/[//]/`는 문자 클래스라 여기까지 오지 않는다.
 * 걸리는 경우(`/a//`)에도 대가는 주석을 덜 지우는 것뿐이다.
 */
 ...
 *
 * **`/`를 만나면 어느 쪽인지 판정하지 않는다 — 안 지우는 쪽으로 넘어진다.**
 * `//`도 `/*`도 아닌 맨 `/`는 정규식 리터럴을 여는 자리일 수 있다(문법상 정규식은
 * 그 두 표기로 시작할 수 없으므로, 모든 정규식 리터럴의 첫 글자는 맨 `/`다).
 * 그것이 정규식인지 나눗셈인지 가리려 들지 않고 둘 다 같이 다룬다:
 *
 * - 같은 줄에서 닫는 `/`를 찾으면 그 구간을 **원문 그대로 건너뛴다** — 안에 있는
 *   `//`·`/*`는 주석으로 읽히지 않는다.
 * - 못 찾으면 **그 줄의 나머지를 통째로 원문 그대로 둔다.**
 *
 * 두 갈래 다 지우지 않는다. 그리고 **닫는 자리가 `//`·`/*`를 여는 `/`면 `regexEnd`가
 * 닫지 않는다**(그 머리 참조) — 그래서 커서가 주석 토큰 한가운데에 서는 일이 없고,
 * 이 함수가 글자를 지우는 자리는 문자열 밖이면서 맨 `/`가 연 구간 밖인, **자기 첫
 * 글자가 곧 토큰의 시작인** `//`·`/*`뿐이다.
 *
 * **한계 — 정직하게 적는다.** 이것은 손으로 만든 스캐너이지 렉서가 아니다(공유 렉서
 * `comment-lexer.mjs`는 `typescript` 임포트 비용 때문에 이 게이트가 쓰지 않는다).
 * **문법상 유효한 소스에서** 남는 한계는 전부 **덜 지우는 쪽**이고, 네 갈래다:
 *
 * - 나눗셈 `/`도 정규식 후보로 다루므로, 한 줄에서 나눗셈 뒤에 붙은 주석은 안 지워질
 *   수 있다. 그 주석에 적힌 모듈명·선언은 위반으로 서서 **붉게** 드러난다.
 * - 그 나눗셈 뒤에 붙은 것이 **블록 주석**이어도 마찬가지다 — 닫는 후보가 `/*`를 여는
 *   자리라 `regexEnd`가 `-1`을 돌리고, 그 줄의 나머지가 원문으로 남는다.
 * - **안 닫힌 정규식 후보**(같은 줄에 닫는 `/`가 없다)도 그 줄의 나머지를 원문으로 남긴다.
 * - **안 닫힌 문자열은 다르다** — `literalEnd`가 `-1`이면 커서는 **한 글자만** 전진하고
 *   스캔이 그대로 이어지므로, 그 줄의 `//`는 정상적으로 지워진다. 문법상 유효한 소스에서
 *   문자열 밖의 짝 없는 따옴표는 리터럴이 아니라 표기(아포스트로피)이므로 이쪽이 옳다.
 *   문법이 이미 깨진 소스에서만 파서가 문자열로 볼 구간이 지워진다.
 *
 * 즉 문법이 성립하는 한 오인의 대가는 주석을 덜 지우는 것(시끄러운 오탐)이지 코드를
 * 지우는 것이 아니다. 한쪽으로만 넘어지므로 이 검사가 조용히 느슨해지는 경로가 없다.
 * (2026-08-23 T-012 — 그 전 판본은 정규식 리터럴을 몰라 `/https?:\/\//` 같은 줄의
 * 실물 임포트를 지웠다. 같은 날 `K-244` — 그 보강판도 나눗셈이 연 유령 구간이 실물
 * 주석의 첫 `/`에서 닫히는 바람에 EOF까지 지웠다. 둘 다 게이트가 코어 예산 위반을
 * 조용히 통과시킨 형태다.)
 */
function stripComments(source) {
  ...
  while (cursor < source.length) {
    ...
    if (char === "/") {
      // 맨 `/` — 정규식 리터럴을 여는 자리일 수 있다. 어느 쪽인지 가리지 않고
      // 둘 다 안 지우는 쪽으로 넘어간다(위 머리의 설명).
      const end = regexEnd(source, cursor);
      if (end === -1) {
        const newline = source.indexOf("\n", cursor);
