 ...
 *
 * 두 갈래 다 지우지 않는다. 그래서 이 함수가 글자를 지우는 자리는 문자열 밖이면서
 * 맨 `/`가 연 구간 밖인 `//`·`/*`뿐이고, 정규식 리터럴은 정의상 전부 그 구간 안에
 * 있으므로 **정규식 안의 `//`·`/*`가 주석으로 읽혀 같은 줄의 실물 코드를 함께
 * 지우는 경로가 없다.**
 *
 * **한계 — 정직하게 적는다.** 이것은 손으로 만든 스캐너이지 렉서가 아니다(공유 렉서
 * `comment-lexer.mjs`는 `typescript` 임포트 비용 때문에 이 게이트가 쓰지 않는다).
 * 남는 한계는 전부 **덜 지우는 쪽**이다:
 *
 * - 나눗셈 `/`도 정규식 후보로 다루므로, 한 줄에서 나눗셈 뒤에 붙은 주석은 안 지워질
 *   수 있다. 그 주석에 적힌 모듈명·선언은 위반으로 서서 **붉게** 드러난다.
 * - 안 닫힌 문자열·안 닫힌 정규식 후보도 같은 줄의 나머지를 원문으로 남긴다.
 *
 * 즉 오인의 대가는 언제나 주석을 덜 지우는 것(시끄러운 오탐)이지 코드를 지우는 것이
 * 아니다. 한쪽으로만 넘어지므로 이 검사가 조용히 느슨해지는 경로가 없다.
 * (2026-08-23 T-012 — 그 전 판본은 정규식 리터럴을 몰라 `/https?:\/\//` 같은 줄의
 * 실물 임포트를 지웠고, 이 게이트가 코어 예산 위반을 조용히 통과시켰다.)
 */
