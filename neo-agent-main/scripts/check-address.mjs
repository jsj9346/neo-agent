 ...
 * `existsSync`/`readdirSync`를 쓰지 않는다 — §7이 재현본을 `git archive` **+ 추적 오라클
 * 복원**(`git init && git add -A && git commit`)으로 만들므로 그 재현본에는 추적 안 되는
 * 파일이 **애초에 없다**. 그런 파일에 기대 풀리는 주소는 디스크로 물으면 작업 폴더에서만
 * 초록이고 재현본에서 죽는다 — 복원 단계는 오라클을 되돌려 줄 뿐 그 비대칭을 안 지운다. 형제
 ...
 * 만드는데 그 재현본에 `node_modules`가 없다. 재현 방법이 그 뒤에 추적 오라클을 복원해도(§7)
 * 재현본 최상위의 `.gitignore`가 그것을 인덱스 밖에 두므로 이 근거는 안 무른다.
 * **갈리는 원소는 오늘 `node_modules` 하나이고 디스크 기준을 쓰면 위반이 9
 * 늘어난다**(`plans/20260903-public-tree-address-audit.md` §7.4).
