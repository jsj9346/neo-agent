...
2. 모드 확인         — "off"면 여기서 allow (하드라인·denied 뒤라는 위치가 중요)
3. 사용자 deny 규칙   — 난독화 정규화 후 글로브 매칭. 걸리면 block
4. 정책 매트릭스     — 자동 허용 판정 (SAFE-DEFAULTS §1: 워크스페이스 안 파일 읽기만).
                      해당하면 allow
5. 위험 패턴 플래그   — 유계 필러 정규식. 차단이 아니라 플래그:
                      allowlist 숏컷을 무효화하고 프롬프트에 경고를 싣는다
...
- **deny 규칙(3)** — 문법은 글로브. 매칭 전에 난독화 정규화를 적용한다: **비가시 유니코드 검사 → NFKC 정규화의 순서 고정**(hermes `threat_patterns.py` 기법 — 순서를 바꾸면 전각 동형이의어 우회가 뚫린다), 인용부호 삽입 변형(`git st""atus`)·백슬래시 변형(`r\m`) 정규화.
- **위험 패턴(5)** — 정규식은 **유계 필러**(`(?:\w+\s+){0,8}` 형태)만 쓴다 — ReDoS 방지. 구체 패턴 목록은 구현 시 확정(크리덴셜 경로 접촉 명령이 여기 포함된다 — TOOLS-INTERFACE §3의 셸 한계 보완).
...
- **allowlist 키 정규화 규칙** — 명령에서 키를 어떻게 추출하는지(첫 토큰? 서브커맨드 포함?). 구현 시 확정하되, 연산자 포함 명령의 학습 불가(§2)는 계약이다.
