...
- **정책 매트릭스(5)** — 자동 허용 대상은 둘이다: **워크스페이스 안 파일 읽기**(`fileRead` + `scope: "inside"`)와 **`memoryWrite`**(2026-08-09 추가 — `MEMORY.md` §5).
  ...
  **`memoryWrite`가 자동 허용인 근거는 "가시성은 성립하고 마찰은 요구되지 않았다"이다.** 승인 매트릭스(SAFE-DEFAULTS §1)가 승인을 요구하는 것은 **되돌리기 어렵거나 범위를 알 수 없는 행동**인데, 메모리 쓰기는 둘 다 아니다 — 몇 KB 상한의 append이고(`MEMORY.md` §6), 사용자가 에디터·`/memory`로 언제든 지울 수 있으며, CLI가 도구 호출과 결과를 그대로 렌더링하므로 사용자가 저장된 내용을 본다. `write_file`이 승인 대상인 이유(비가역 덮어쓰기)가 여기엔 없다. `ARCHITECTURE.md` §2.6이 요구하는 것은 가시성이지 마찰이 아니다.
  ...
  **자동 허용에 예외를 만들지 않는다** — 위험 플래그(4)·오염 플래그(4b)가 있으면 `memoryWrite`도 다른 자동 허용과 똑같이 무효화되어 프롬프트로 간다. 오염 런에서는 도구 자신이 별도로 거부하므로(`MEMORY.md` §5) 사용자가 허용해도 저장되지 않는데, **이 이중 마찰은 수용한다**: 4b를 `memoryWrite`에 한해 면제하면 "4b는 플래그이지 출구가 아니다"(판정 C-6)를 뒤집게 된다. 대신 `display`가 **무엇이 저장되려는지** 보이게 해 프롬프트가 무의미해지지 않도록 한다(§4).
...
- **프로필과 subject 양쪽에 인자 필드가 없다.** `remember`의 인자는 `content` 하나이고 **경로 인자가 아예 없다**(`MEMORY.md` §4.1 — 대상 파일은 `deps.dir`로 고정이고 모델 입력에서 파생되지 않는다). 그래서 `pathParam`·`urlParam`에 대응하는 것이 없고, 판정 대상은 **"이 도구가 불렸다"는 사실 하나**다. `shellExec`·`webFetch`에 `scope` 판정이 없는 것과 같은 자리 — 게이트가 읽을 수 있는 판정 축이 그 도구에 존재하지 않을 때 축을 발명하지 않는다.
