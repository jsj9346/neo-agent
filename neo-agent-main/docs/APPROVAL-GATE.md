...
**`packages/gate`의 공개 계약 정본.** 작성일: 2026-08-06 · 최종 개정: 2026-08-13(머리 — 구현 완료로 정정) · 상태: 설계 확정, **구현 완료**.

> **2026-08-13 정정** — 이 줄은 2026-08-06 구현(`packages/gate`, `51eeb2e`) 이후 7일간 *"구현 전"*으로 남아 있었다. `SANDBOX.md`:5가 2026-08-12에 지목한 6개 문서 중 하나이며, 전수 대조 결과 6개 전부가 같은 부정확이었다 — 근거는 `plans/20260813-docs-headers-verify-report.md` F-1.
