...
| `compactionAuto` | `boolean` | `true` | 압축 자동 트리거 (`COMPACTION.md` §3) — 2026-08-06 개정 ⚠️ 구현 전 |
| `compactionThreshold` | `number` | `0.75` | 압축 임계 비율 — 2026-08-06 개정 ⚠️ 구현 전 |
| `compactionKeepRecentTurns` | `number` | `2` | 압축 시 원문 유지 user 턴 수 — 2026-08-06 개정 ⚠️ 구현 전 |
...
| `/compact` | 수동 압축 — 자동 트리거와 같은 경로, 임계 미달이어도 실행 (`COMPACTION.md` §3·§6). 2026-08-06 개정 ⚠️ 구현 전 |
| `/exit` | 종료 시퀀스 (§2) |
...
- **자동 압축의 판정 시점은 CLI가 소유한다** (2026-08-06 압축 설계 — ⚠️ 구현 전): 런 종료 후 idle과 재개 직후 2곳에서 판정하고, 압축 시 Agent 교체는 `/resume`·`/new`와 같은 "폐기 후 재생성" 경로다. 압축 발생·결과·실패는 침묵하지 않는다. 상세는 `COMPACTION.md` §3·§6·§7.
