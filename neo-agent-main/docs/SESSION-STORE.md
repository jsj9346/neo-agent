...
| FTS5 + 트라이그램 (IDEA-004) | ~~후순위, 흔적 불필요~~ → **2026-08-07 채택** (`SEARCH.md`, 스키마 v3) | "흔적 불필요"(`1a53dd6^`) 판정(파생 데이터라 스키마 흔적이 필요 없다)은 적중 — v1·v2에 아무 흔적 없이 v3에서 테이블 추가만으로 도입됐다. 단 "external-content 테이블"이라는 예상 형태는 **개정**: body가 JSON 통짜라 독립 테이블 + 앱 코드 색인이다(`SEARCH.md` §2) |
