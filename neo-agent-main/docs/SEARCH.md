...
- 상태: 구현 완료
- 근거: packages/store/src/search.ts
- 작성일: 2026-08-07 (IDEA-004 채택 — devlog 같은 날)
- 최종 개정: 2026-08-07 구현 착수 실측 (§4 스니펫 생성 수단 정정 — `snippet()`이 trigram과 조합 불가함이 실측됨) · 2026-08-13(머리 — 지위 선언을 필드로, `DOC-STATUS.md` §3)

> **앵커가 `packages/store/src` 전체가 아니라 파일 하나인 이유** — 검색은 store 패키지 안에 살지만 그 패키지의 정본은 `SESSION-STORE.md`다. 두 문서가 같은 앵커를 들면 어느 쪽이 무엇을 주장하는지 흐려진다. 2026-08-13에 `CLI-INTERFACE.md` 머리의 *"검색 제외"*(`d98b223^`)가 거짓으로 판정된 것(F-2)이 이 경계가 흐렸던 결과다.
...
**`id`가 NULL인 메시지 행은 색인에서 제외하고 통지한다** (2026-08-07 구현 판정 ES-32). **그런 행은 존재할 수 없다** — `STRICT` 테이블은 PK 열에 NOT NULL을 그대로 강제하고(NULL을 허용하는 SQLite의 오래된 예외는 **비STRICT 테이블에만** 적용된다 — 실측표는 `schema.sql.ts`의 E-21), `messages`는 v1부터 STRICT였다. 그럼에도 분기를 두는 근거는 **SQLite 행이 TS 경계에서 `unknown`이라는 것**이다: 타입을 좁히는 일은 어차피 해야 하고, 좁히다 실패했을 때 조용히 NULL을 색인하면 `message_id`가 NULL이 되어 §4의 dedupe 기준(같은 id는 1회)이 무너진다. 도달하지 않는 것이 정상인 가지이며, 제외는 조용하지 않다 — 손상 행과 같은 경고 경로로 통지한다(§6).
...
- ~~**한국어 검색 품질의 합격 기준.**~~ → **2026-08-07 해소.** 실측 리포트(`plans/20260807-search-quality-report.md`, QA-B)의 제안을 채택한다. 기준 7개 전부 현행 구현에서 통과했다.
  ...
  **핵심 실측**: 어근 질의가 모든 활용형을 잡고(재현율) 활용형 질의는 그 활용형만 잡는다(정확도) — **한국어 조사 처리에 형태소 분석기가 필요하지 않다**는 것이 trigram 채택의 실측 근거다. 이에 따라 §7의 `hermes CJK 네이티브 확장` 행이 든 재도입 트리거 *"한국어 검색 품질 미달 실증"*은 **발동하지 않는다.**
