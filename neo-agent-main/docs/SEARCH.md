...
> **앵커가 `packages/store/src` 전체가 아니라 파일 하나인 이유** — 검색은 store 패키지 안에 살지만 그 패키지의 정본은 `SESSION-STORE.md`다. 두 문서가 같은 앵커를 들면 어느 쪽이 무엇을 주장하는지 흐려진다. 2026-08-13에 `CLI-INTERFACE.md` 머리의 *"검색 제외"*가 거짓으로 판정된 것(F-2)이 이 경계가 흐렸던 결과다.
