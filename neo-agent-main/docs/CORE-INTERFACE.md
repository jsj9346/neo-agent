...
7. **idle이면 양쪽 큐는 비어 있다** (2026-08-06 추가) — 큐 항목은 런 경계를 넘지 못한다. 진입 차단(idle의 `steer()`/`followUp()`은 throw, §4)과 탈출 보장(런 종료 직전 steering 드레인·비정상 종료 시 큐 클리어, §5)이 양쪽에서 이를 지킨다. 먼저 큐된 메시지가 나중 프롬프트 뒤에 주입되는 순서 역전이 계약 수준에서 불가능해진다.
...
확정해 문서에 반영한 것(더는 미결 아님): `errorMessage`의 적용 범위(§2), followUp이 런을 쪼개지 않음(§4), 리스너 예외 의미론(§3), `io: "input"` 변환·`done` 없는 스트림 처리(§8), 도구 등록 fail-fast·`validateToolArgs` 공개(§6). **2026-08-06 해소: O-1(턴 한도 — `maxTurnsPerRun` + grace 턴, §5)·O-3(steering 큐 — 종료 직전 드레인 + idle-throw, §4·§5·불변 조건 7).** 번호는 재사용하지 않는다 — 과거 기록(devnote·QA 리포트)이 이 번호를 참조한다.

> ⚠️ **계약-구현 격차**: O-1·O-3 해소는 2026-08-06 기준 **문서 확정**이며 `packages/core` 구현은 아직 구 동작(상한 없음, idle 큐 잔류)이다. CLI 플랜 착수 전에 구현을 이 계약에 맞춘다.
