...
- 상태: 구현 주장 없음
- 작성일: 2026-08-05
- 최종 개정: 2026-08-13(머리 — 지위 선언을 필드로, `DOC-STATUS.md` §3) · 2026-09-03(「판정 등급」 표·§5 — 금지 등급의 관할과 §5 정본 주소를 `COMPLIANCE.md`로. 증거의 자리도 그 문서로 정정) · 2026-09-03(§2.8 — 주소 표기 정정. hermes의 런타임 산물 USER.md의 코드 스팬을 걷는다(`PUBLIC-TREE.md` §3.4 첫 행)) · 2026-09-06(**§2.9 — `doctor` 한 항목을 기각에서 분리해 채택으로 뒤집었다.** 정본은 `CLI-INTERFACE.md` §5.1. 원 기각(2026-08-09)의 근거 둘 중 「argv를 4개로 닫은 기결정」은 `serve` 편입(2026-08-25)으로 이미 죽어 있었고, 「doctor = 시작 시퀀스의 검사들」은 절반만 산다 — 시퀀스가 재는 것은 축 여섯 중 넷이고 첫 실패에서 멈춘다. **나머지 넷(`onboard`·`update`·`uninstall`·`migrate`)의 기각은 그대로다** — 그 넷은 수명주기 조작이고 대응물이 실재한다)
...
- **설정 명령 표면은 hermes의 `"Configuration"` 카테고리 넷을 명령 하나로 축소해 채택한다** (2026-09-05). 런타임 전환 의미(`/approvals`·`/yolo`)와 스코프 인자(`--once`/`--session`/`--global`)는 **안 가져온다** — 위 「설정 스냅샷 원칙」이 그것을 배제하고, 우리 `/config set`은 파일만 바꾸며 적용이 다음 시작이다. **판정 전문과 근거는 `CLI-INTERFACE.md` §3.2·§11이 든다** — 여기 다시 적지 않는다.
...
- **CLI 운영 명령 20종 중 `doctor`를 뺀 나머지**(`onboard`·`setup`·`update`·`uninstall`·`migrate`…) — 전부 **수명주기 조작**이고 우리 대응물이 명령 아닌 형태로 실재한다: update = `git pull`, migrate = 기동 시 자동, uninstall = 링크 제거. 이 기각은 그대로다.

> **2026-09-06 — `doctor` 한 항목만 이 기각에서 분리해 채택으로 뒤집었다.** 정본은 `CLI-INTERFACE.md` §5.1이고 상위 정본은 `plans/20260906-doctor-command-interview.md`(상태: 결정됨)다. 원래 문면은 20종을 한 묶음으로 기각하며 근거 둘을 들었다 — *"`CLI-INTERFACE.md` §5가 argv를 4개로 닫은 기결정과 정면으로 충돌한다"*와 *"doctor = 시작 시퀀스의 fail-closed 검사들"*.
>
> **첫째 근거는 이미 죽어 있었다**: `serve`가 2026-08-25에 다섯 번째로 들어왔고, §5 자신이 *"닫힌 목록 — 추가는 이 문서 개정"*이라 적는다 — 금지가 아니라 개정 절차의 문제다. **둘째 근거는 절반만 산다**: 시작 시퀀스가 실제로 재는 것은 §5.1의 축 여섯 중 넷뿐이고(이미지 실재·검색 열쇠는 안 잰다), 그나마 **첫 실패에서 멈춘다.** 그리고 `doctor`만 성격이 다르다 — 나머지 넷이 수명주기 조작인 데 반해 이것은 **내성(introspection)**이라 「대응물이 있다」는 주장의 성질 자체가 다르다.
>
> **이 문서의 §6 규칙대로 devlog가 먼저다** — 판정 전환의 근거는 2026-09-06 항목이 든다.
